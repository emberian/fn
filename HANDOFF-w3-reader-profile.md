...
worktree**. Everything above is a statement of what the books say, not of what
ACL2 has checked. The next owner should run

```
FN_ACL2_TIMEOUT_SECONDS=1800 python3 tools/certify_books.py \
  books/nntp books/nntp-overview books/nntp-invariants books/nntp-effects \
  tests/acl2/nntp-tests tests/acl2/nntp-teeth-tests \
...
`e/d` disable lists on the four composed effect theorems, which now have to keep
five more branch functions closed.
