...
4. The refusal tables of docs/install.md.  The region between
   `<!-- generated by docs_check --write: reasons ...-->` and
   `<!-- end generated reasons -->` is written from the books' own tables:
   the health states and their exit codes (books/native-health.lisp,
   `*fn-nh-states*`, `*fn-nh-first-exit*`) and the POST refusal reasons with
   the reply each sends (books/nntp-post.lisp, `fn-post-refusal-line`).  The
   advice column for a health state is HEALTH_ADVICE below; a state the book
   gains without advice, or advice for a state it lost, fails `--check`, and
   so does a region that is not what the books say now.
...
# ---------------------------------------------------------------------------
# 4. the refusal tables of docs/install.md

INSTALL_DOC = ROOT / "docs" / "install.md"
REASONS_BEGIN = "<!-- generated by docs_check --write: reasons (do not edit by hand) -->"
REASONS_END = "<!-- end generated reasons -->"
HEALTH_ADVICE = {
    "fenced": "a process holds the store (the node is starting, or another command runs), "
              "or the owner does not answer: wait and ask again; never delete the lock",
    "exhausted": "the store reached a counter's ceiling (a transaction id or the retention "
                 "count); nothing raises it in place",
    "unqualified-profile": "the store runs the development profile, which is for tests: "
                           "make the node again with a `mission`",
    "space-pressure": "free headroom is low on transactions, history or retention: `capacity` "
                      "says which; release obligations or move to a larger profile",
    "no-route": "forwarding obligations wait and no BP route is configured",
    "stranded-transfer": "a peer refused an article past its retry bound: fix the peer",
    "unavailable-peer": "a peer has articles waiting and no connection: check its address "
                        "and port (`peer list`) and that it is up",
    "receipt-debt": "forwarding obligations wait for the receipts that release them",
}
...
def health_states():
    ...
    states = re.search(r"\(defconst \*fn-nh-states\*\s+'\(([^)]*)\)\)", text).group(1)
    first = int(re.search(r"\(defconst \*fn-nh-first-exit\* (\d+)\)", text).group(1))
    unobserved = int(re.search(r"\(defconst \*fn-nh-unobserved-exit\* (\d+)\)", text).group(1))
...
def reasons_region():
    ...
    lines = [REASONS_BEGIN, "",
             "`health` exits with the code of the first condition held, in this order "
             "(0: every condition clear; %d: some condition could not be observed, as the "
             "two feed conditions cannot while the node is stopped):" % unobserved, "",
             "| exit | condition | what it means, what to do |", "| --- | --- | --- |"]
    for index, state in enumerate(states):
        lines.append("| %d | `%s` | %s |" % (first + index, state,
                                              HEALTH_ADVICE.get(state, "(no advice)")))
    lines += ["", "A refused POST is answered `441` with the reply below, and the log "
              "line for it starts `refused` and names the reason word:", "",
              "| reason | the reply |", "| --- | --- |"]
    for reason, reply in post_refusals():
        lines.append("| `%s` | `%s` |" % (reason, reply))
    lines += ["", REASONS_END]
    return "\n".join(lines)


def check_reasons(write):
    failures = []
    states, _first, _unobserved = health_states()
    for state in states:
        if state not in HEALTH_ADVICE:
            failures.append("health state %s (books/native-health.lisp) has no HEALTH_ADVICE "
                            "in tools/docs_check.py" % state)
    for state in HEALTH_ADVICE:
        if state not in states:
            failures.append("HEALTH_ADVICE names %s, which books/native-health.lisp no "
                            "longer has" % state)
    text = INSTALL_DOC.read_text(encoding="utf-8")
    start = text.find(REASONS_BEGIN)
    end = text.find(REASONS_END)
    if start < 0 or end < start:
        return failures + ["docs/install.md has no generated reasons region"]
    region = reasons_region()
    current = text[start:end + len(REASONS_END)]
    if write:
        if current != region:
            INSTALL_DOC.write_text(text[:start] + region + text[end + len(REASONS_END):],
                                   encoding="utf-8")
            print("wrote the reasons region of docs/install.md")
    elif current != region:
        failures.append("docs/install.md's reasons region is not what the books say now: "
                        "run tools/docs_check.py --write")
    return failures
