#!/bin/sh
# Drive tin 2.6.2 against an fn node through tools/nntp_wire_log.py (reader spike, D28).
#   spike_tin_drive.sh TIN WIRE_PORT HOME_DIR OUT_DIR STEP...
...
tmux new-session -d -s spiketin -x 120 -y 40 \
  "env HOME=$home TERM=xterm EDITOR=$home/editor.sh VISUAL=$home/editor.sh $tin -r -A -p $port -g 127.0.0.1 2>$home/tin.stderr; sleep 3600"
sleep 8
n=0
