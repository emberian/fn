...
; On a retained article history, the second allocator run's successful
; os.replace/root fsync installs exactly the host-supplied frame.
(assert-event
 (let* ((entry (bsk5-finished))
        (octets (bsk0-second-frontier-host-frame))
...
(assert-event
 (with-guard-checking :none
  (let* ((bs (bsk5-initial))
        (oc0 (bsk0-owner-frontier-entry))
        (s0 (fn-own-store (fn-ocfg-owner oc0)))
        (s (fn-sn-update s0 (fn-sn-files s0) :bad))
        (oc (fn-ocfg-make (fn-own-start s 2) nil nil nil))
        (stage ".allocation-owner-eio-k0")
        (octets (fn-bs-frontier-encode 1)))
...
(assert-event
 (mv-let (result bs)
   ...
   (declare (ignore result))
   (let* ((oc (bsk0-owner-frontier-entry))
          (s (fn-own-store (fn-ocfg-owner oc)))
          (stage ".allocation-owner-eio-k0")
          (octets (fn-bs-frontier-encode 1)))
     (and (fn-sn-statep s)
