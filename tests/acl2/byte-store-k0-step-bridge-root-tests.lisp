...
; root name (fn-bs-k0s-root-rename-pendingp) at the state checkpoint's
; pending rename, the program's cuts by the keystone
; (fn-bs-k0-state-checkpoint-cuts-relation-by-step) and every step at its
; pairs with any outcome.  Witness: the second publication's entry pair
; (bskc-rb, bskc-rk), related, outside the recovery window, :root and
; :transactions quiet; the checkpoint octets arbitrary.  (The profile
; program's rows went with P-PROFILE, D34.)
(in-package "ACL2")
