...
| RFC 5537 §3.5, the operator's submission | `fn operator CONFIG post` hands the owner a proto-article: the node is its injecting agent exactly as for a served POST | Implemented 2026-09-22 as the owner event `(:operator-submit ...)`, `fn-own-operator-submit` (`books/owner.lisp`), which host/owner-host.lisp `fn-owner-operator-submit` steps and host/native/owner.lisp `fnn-owner-control-submit-serialized` calls after one clock reading. It injects through `fn-inj-decide` under the owner's posting configuration and clock; refuses without a usable clock (D10-a, `:clock-unusable`); refuses a command-line Message-ID or newsgroup list the injected article does not carry (**refusal word** `:control-mismatch`, operator exit 1 `refused`); and when the live node already holds that Message-ID as an injection of the same octets by this agent (`fn-inj-reinjectionp`), submits the stored article so the store answers duplicate after a lost outcome. Every other injection refusal keeps its word (`:from-invalid`, `:path-present`, ...), reported to the operator as refused (the control reply carries the status, not the word). Until then the verb stored the payload as read, with no Path, and INN refused its feed `437 Missing "Path" header field` (planning/evidence/inn-lab-dabebb84-2026-09-22.md, finding 1). Proved in `books/owner-invariants.lisp`: `fn-own-operator-submission-is-an-injection-of-the-payload`, `fn-own-operator-decision-is-an-injection-of-the-payload`, `fn-own-operator-submit-without-a-clock-refuses-and-changes-nothing`, `fn-own-operator-retry-resubmits-the-stored-injection`, over `fn-inj-injected-article-is-a-reinjection-of-its-source` (`books/injection-invariants.lisp`); teeth in `tests/acl2/owner-operator-tests.lisp` from the lab's own octets. The hybrid-signed author path and BP application submissions keep the exact-octets `fn-own-control-submit` |
| RFC 5536 §3 mandatory fields; §3.2.6, §3.2.8, §3.1.5 | Date, From, Message-ID, Newsgroups, Path, Subject; Injection-Date; Injection-Info; Path | From, Subject and Newsgroups must be supplied; Message-ID, Date, Injection-Date, Injection-Info and Path are generated by ACL2 and by nothing else. Local policy, not an RFC requirement: a proto-article that already carries Path or Injection-Date is refused rather than rewritten, which is what makes the verbatim-suffix theorem true. The same injection answers the served POST and, since 2026-09-22, `fn operator CONFIG post` (row RFC 5537 §3.5 below). Open: `fn-af-message-idp` of every generated identifier is witnessed, not proved; and that a different millisecond reading yields a different instant (the two calendar inverse lemmas and the rendering injectivity are proved in `books/injection-invariants.lisp`) |
| RFC 5536 §3.1.2 (with §§2.1, 2.2) | From is an RFC 5322 §3.4 `mailbox-list`: no obsolete syntax but `obs-phrase`, US-ASCII | Implemented in `books/mailbox.lisp` (`fn-mbx-mailbox-listp`, a bounded two-pass recognizer: a lexer to token kinds with comment nesting as a counter, one call per octet, then an automaton over the kinds; the value is bounded by the header block before either pass and nothing reads through the Lisp reader) and checked by `fn-inj-mandatory-reason` after `:from-missing` and `:from-duplicate`. **Refusal word** `:from-invalid`, **441 line** `441 posting failed; From is not a valid mailbox list` (`fn-post-refusal-line`, `books/nntp-post.lisp`). Until 2026-09-22 only presence was checked and `From: yue` was answered 240 and served (planning/evidence/agents-on-hbox-2026-09-22.md, run 03). Proved: every accepted value names an address (`fn-mbx-mailbox-list-names-an-address`), and every injected proto-article's From is a mailbox-list naming one (`fn-inj-injected-proto-article-has-a-mailbox-list-from`). Tested: accepted and refused values across the grammar's productions, the lab's own article and `From: yue`, in `tests/acl2/injection-tests.lisp`; the v0 matrix row V0-POST-FROM-MAILBOX measures it on a node. Not claimed: that the recognizer accepts every conformant value (no completeness theorem; the witnesses are the evidence), group syntax is refused by design (From is a mailbox-list, not an address-list) |
...
| §7.4 | NEWNEWS filtering/time forms | Implemented over the durable acceptance stamp and the existing §7.3.2 date/time and wildmat parsers. A schema-0 article uses the nearest later natural stamp, then the pinned reader wall second, then an unbounded legacy horizon. `fn-nntp-newnews-scan-is-the-acceptance-filter` proves complete soundness/completeness against an independent specification; `fn-nntp-newnews-scan-reads-no-payload`, `fn-nntp-newnews-scan-lines-at-most-candidates`, and `fn-nntp-newnews-lines-are-clean` give the served cost and block properties. The dispatcher equation and session-preservation theorem remain. The former 256-parse 503 refusal is removed; 503 remains for a two-digit year with no reader wall. |
| §7.6.1 | LIST defaults, keyword variants, syntax/availability errors, no state changes | Default, ACTIVE (with and without a wildmat), ACTIVE.TIMES, NEWSGROUPS (with the configured descriptions, NNT-039), MOTD (RFC 6048 §2.5, NNT-039), HEADERS and OVERVIEW.FMT implemented; DISTRIB.PATS and DISTRIBUTIONS are recognized-but-unmaintained 503; malformed or unknown variants are 501. The variant keyword is dispatched by `fn-nntp-list-command`, which is what `books/nntp.lisp` calls, so that ACTIVE.TIMES can read the environment; session preservation proved (`fn-nntp-list-response-preserves-session`) |
...
| NEWNEWS *wildmat* *date* *time* | `O(A · G')` for `A` committed articles and `G'` matched groups, one stamp comparison per article, no article payload read, at most `C` lines for `C` matched-group candidates; no size refusal. |
...
| No LIST ACTIVE.TIMES, LIST NEWSGROUPS or LIST MOTD line carries CR, LF or NUL | proved (`fn-nntp-active-times-lines-are-clean`; `fn-nntp-newsgroup-lines-are-clean` under `fn-nntp-safe-group-listp`; `fn-nntp-described-lines-are-response-text`, `fn-nntp-motd-lines-are-response-text`) |
...
| A NEWNEWS answer names exactly the articles of the matched groups at or after the acceptance stamp or conservative legacy horizon | proved (`fn-nntp-newnews-scan-is-the-acceptance-filter`) against an independent quadratic specification |
| A NEWNEWS payload is never parsed and no size budget truncates the answer | proved (`fn-nntp-newnews-scan-reads-no-payload`, `fn-nntp-newnews-scan-lines-at-most-candidates`) |
| An undated or malformed article can still be reported | tested on natural stamps, a mixed legacy list, a wall-less reader and a 32 KiB unparseable payload (`tests/acl2/nntp-newnews-tests.lisp`) |
...
| §7.6.6 LIST NEWSGROUPS | Group name, separator, short description; description may be passed on as held | Implemented (NNT-039, PRF-195): the second field is the group's description from the configuration's descriptions slot, set by `group describe` and published live (`fn-served-step-list-newsgroups-is-the-described-listing`, `fn-oag-description-of-the-listing`), and the fixed marker `(no description)` for a group with none. The marker is **not** the empty string, and that is measured: Python nntplib strips the line and then requires name + white space + text, so a bare `name TAB` drops the group from `descriptions()` entirely (`tests/interop_nntplib.py`, 2026-09-20); admission refuses a description of spaces only for the same reason. Lines proved clean (`fn-nntp-newsgroup-lines-are-clean`, `fn-nntp-described-lines-are-response-text`); native case `tests/test_native_reader_index.py` `test_list_newsgroups_descriptions_and_motd_published_live` |
...
| §2.1.6 LIST NEWSGROUPS | Name and description, optional wildmat | implemented with the configured description or the `(no description)` marker; see the §7.6.6 row |
...
| --- | --- | --- |
| §2.1 | Advertise `AUTHINFO USER` only for what the server will accept now | proved: `fn-auth-authinfo-is-not-advertised-once-authenticated-on-any-connection`, over `fn-auth-capability-lines-for-peer`, which is the list the CAPABILITIES arm builds for every connection; `fn-auth-authinfo-is-not-advertised-once-authenticated` is its reader instance, and `fn-auth-step-capability-block-unfolds-to-the-peer-aware-lines` is the named unfold tying the list to what the step emits. Also withheld on an unprotected connection when the configuration is protected-only, because the server would answer 483, and withheld when the configuration holds NO credential, because every PASS would then be 481. The SASL argument is never advertised. Live since 2026-09-21: the label appears in `CAPABILITIES` on a node started `fn run` with a credential the CLI wrote, on a reader connection and on one the owner resolved to a peer record alike (`fn-served-peer-and-reader-open-under-the-same-policy`, PRF-039) |
...
the handshake. It emits a `(:starttls)` effect; the native owner delegates the
handshake and protected records to OpenSSL 3, while the development adapter
uses Python's `ssl` module. What the books prove is the protocol state machine
around the upgrade and the native plaintext-prefix ownership, not TLS.

| RFC 4642 clause | Requirement | Proved / tested / trusted |
| --- | --- | --- |
| §2.1 | Advertise the STARTTLS label; MUST NOT advertise it once a TLS layer is active | **proved**: `fn-auth-starttls-is-not-advertised-under-tls-on-any-connection` (2026-09-22, over `fn-auth-capability-lines-for-peer`, so a client the owner resolved to a peer record is inside the claim), with `fn-auth-starttls-is-not-advertised-under-tls` as its reader instance, and `fn-auth-starttls-is-not-advertised-without-a-certificate` for the case where no certificate is configured. None of the three says anything about the octets `fn-nntp-multi` renders the list into |
...
| §2.2 | STARTTLS MUST NOT be pipelined; the handshake begins with the first octet after the 382's CRLF | **proved for protocol framing.** `fn-served-feed` (`books/served.lisp`) stops on a handshaking connection exactly as it stops on a closed wire (`fn-served-feed-of-handshaking-connection`, `fn-served-step-of-handshaking-connection-is-a-no-op`), so octets behind the command line in the same observation are never framed as NNTP. For native robustness when a nonconforming peer sends ClientHello early, `fn-ocfg-read-tls-prefix-is-full-read` equates the actual owner call on the chosen prefix with the whole observation and `fn-served-tls-prefix-suffix-accounting` preserves the suffix for OpenSSL. This does not permit clients to pipeline the negotiation. The native adapter trusts sole-reader scheduling and stable `MSG_PEEK` then exact consume, checking short and changed consumption before handing the suffix to OpenSSL |
...
| §2.2.2 | 580 when the server cannot initiate | implemented for the configuration reason (no certificate or key configured). The native operator rejects an unloadable or mismatched configured pair before listening. A peer-specific handshake failure after a 382 closes that connection and leaves the listener/context available; it is not reclassified as 580 because negotiation has begun |
...
| §2.3 | The security layer itself: confidentiality, integrity, certificate validation, cipher selection, Client Hello compatibility, SNI | **trusted, not modelled.** OpenSSL 3 in the native owner and Python's `ssl` in the development adapter use the configured `[listener] tls cert key`. The native library, loader, C ABI, socket BIO and platform readiness behavior are explicit trust. No theorem in this tree says anything about their TLS correctness. Recorded in the trust boundary of [the NNTP contract](nntp.md) |
