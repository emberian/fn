...
| "the peer enumeration" (host `:program` twin) | `fn-cfg-peer-names` (`books/peer-config.lisp`) | **Closed for the host, one logic-mode copy left.** `host/store-node-host.lisp`'s `fn-store-cfg-peer-name-list` was a `:program`-mode copy and is deleted: `fn-store-cfg-peer-names` and the operator's `peer list` report both call the book's fold now. `fn-own-feed-peer-names` is the same fold over the same rows and is still written out in `books/owner-feed.lisp`; it collapses into this one when owner-feed is next recertified. |
