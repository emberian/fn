...
| reference fragmentation/reassembly; cursor cutter and bounded position scanner proved equal on all inputs; whole-parent restoration, fragment/reassemble inverse and consumed-fragment agreement | `books/bp-fragment.lisp`, `books/bp-fragment-invariants.lisp`, `books/bp-fragment-fast.lisp` | scoped C1 certificate in `planning/evidence/bp-fragment-c1-subset-2026-09-23.md`; principal/coherence active-set partition and machine-level N10 remain open |
...
4. A redundant hypothesis is removed, not witnessed. If a lane finds that
   no state satisfies rule 2 for a hypothesis, the hypothesis is redundant:
   it removes it and says so in its evidence file.
...
| Journal debt (§2.1) | teeth book (A1) | N05: from `C + 2N + 1` free, an attempt and its failed result leave the cover intact; at `remaining = D + control-reserve` a discard is admitted | debt-paying hypothesis dropped: a kind 5 at that frontier is refused `:capacity`; mutation: the first revision's rule (failed results exempt, no reservation at kind 8) breaks the inequality on N05's trace |
| Selection (§4.2, §8) | teeth book (A1) | N03: older route-less transit entry and newer local request, the request is delivered within four progress events; N04: older 200 KB entry and newer 10 KB entry on a 64 KB session, the newer is attempted | mutation: the first revision's least-arrival-regardless selector chooses the route-less entry forever on N03; eligibility dropped: N04's older entry is a candidate and not eligible |
