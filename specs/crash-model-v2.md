...
| K5-K8 | **K7 publication uncertainty proved for the four native authority boundaries** in `books/byte-store-fault-keystones.lisp`: issued record link, issued allocator rename, transaction-directory barrier error and root-directory barrier error each enter the recovery fence at the exact composed `fn-sn-io` subject.  The byte transition theorems also state which directory choice remains pending or is resolved, with reachable witnesses and violating cases in `tests/acl2/byte-store-fault-keystones-tests.lisp`.  This is logical syscall/observation correspondence, not filesystem or power-loss qualification.  The exact-octet native SIGKILL scenario awaits the next frozen developer image. **K5, K6 and K8 remain open** (P3). |
