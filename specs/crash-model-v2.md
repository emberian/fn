...
P-PROFILE (2026-09-24, M5), the offline profile upgrade's replacement of
`config.json`, was removed with the upgrade by D34 (fresh deploys): no
program writes `config.json` after `init`, and `store import` builds a new
store beside its path and renames it into place only after the ordinary
open admitted it.
...
(defun fn-bs-replay-matches-scan (bs ks)
  ...
  (let ((scan (fn-bs-scan-store bs))
    ...
    (and (fn-bs-pending-shape-okp bs)
         ...
         ; K2f (2026-09-20, lane w11/bytestore-k2).  The window is entered
         ; with a pending :root rename too, and :root is drained only by the
         ; FOURTH recovery barrier, so the DURABLE frontier is then the
         ; scanned one minus one -- advance_frontier writes old+1
         ; (run_store.py:1281).  (null txn-ops) is the exclusivity the phase
         ; supplies in the publish window and cannot supply here: a dead
         ; process leaves at most one un-fenced authority entry, because
         ; each publishing program fences its own directory before it
         ; returns and none runs before recovery completes.
         (if root-ops
             (and (null txn-ops)
...
; The root-directory EIO cut is covered for both supported pending-entry
; choices.  Given a related ready byte/owner input, a typed successor
; frontier frame and fresh staging,
; fn-bs-k0-frontier-eio-choice-run-has-actual-failed-cut equates pair 12 of
; the actual stopped P-FRONTIER fn-bs-run to its EIO byte result.
; fn-bs-k0-owner-frontier-root-eio-choice-run-fences-related-state joins that
; cut to the exact fn-ocfg-step :store/:io callbacks: the owner is fenced,
; the full byte/kernel relation survives, and the durable frontier is the
; candidate for :apply or the old value for :drop.  The test book reaches
; both choices from the same related input and checks the exact conclusion.
; The host classifies a fnn-fsync-dir root error after attempted rename as
; fnn-indeterminate; no success callback occurs.  Served call-entry relation,
; other error and torn paths, and physical filesystem-barrier qualification
...
;
; K2f, the frontier sub-case.  MODELLED 2026-09-20 (lane w11/bytestore-k2);
; K2 above is now stated over a predicate that covers it, and what is left
; open is K2 itself, not its statement.  The recovery window is also entered
...
; fsync_dir(root) at :1216, the FOURTH recovery barrier -- so a crash there
; rolls the frontier back to the durable one, a value the kernel does not
; hold (fn-sf-frontier-candidate is nil in that window).  Before this,
; K2 as written was FALSE in that sub-case.
;
; fn-sf-recovery-crash-imagep now carries the arm
;   (and (fn-sf-frontier-rollback-visiblep s)
;        (equal frontier (1- (fn-sf-frontier s)))
;        (equal records (fn-sf-records s)))
; with the gate
;   (and (fn-sf-recovery-visiblep s) (null (fn-sf-successes s))
;        (posp (fn-sf-frontier s))
;        (fn-sf-record-listp (fn-sf-records s) 0 0 (1- (fn-sf-frontier s))))
; and fn-sf-crash-frontier-rollback inhabits it.  The record-list conjunct is
; the correspondence and not a convenience: advance_frontier reserves txid =
; old for the record published AFTER the rename is durable, so while the
; rename is pending no record holds that txid, and once it is published the
; conjunct is false and the frontier can no longer roll back.  The same
; conjunct is what keeps fn-sf-recovery-admissible-image-facts true of the
; image, what makes fn-sf-crash-frontier-rollback preserve fn-sf-statep, and
; what makes the image replayable (fn-snt-recovery-admissible-crash-image-
; is-recoverable).  The (equal records ...) conjunct is exclusivity: a
; recovery window holds at most one pending authority entry operation.
; The (null (fn-sf-successes s)) conjunct is NOT about losing a record -- the
; record list does not change -- but about telling a scan-reached :replaying
; state from a crash-reached one: a crash from :reserved has already seen
; (:frontier-directory :ok), so its rename is durable, and a crash from
; :frontier-attempted is covered by fn-sf-crash's :old choice.
; fn-bs-replay-matches-scan already carries it, so K2 pays nothing.
...
;
; SCOPE, proved 2026-09-21 and recorded because it is not visible in the
; statement: K4's instances INSIDE the recovery window are vacuous, since
; fn-bs-replay-matches-scan carries (equal (fn-sf-successes ks) nil) there
; and K4's last hypothesis is a member of that list.  Its live instances are
; the publish window.  What covers the recovery window is the half with no
; success hypothesis at all, fn-bs-crash-image-reopens
; (books/byte-store-keystones.lisp): the host reopen entry succeeds on a
; byte-level crash image of a related state when both the observed identity
; and topic histories replay.  The topic condition is not yet derived from
; the maintained Store/byte relation for every physical crash cut.
; With E2 Store consumer events, the reopen statement also needs the
; maintained fn-csi-full-relationp over the completed Store prefix.  K2's
; scanned image and fn-csi-recovery-crash-image-strict-replay derive the
; consumer replay check for all platform arms, including a recovery-window
; rollback of the last unfenced record.  It is not an extra assumption on
; the observed scan.  Identity and topic replay remain separate observed
; premises.  The latter has a physical FNST-image counterexample when the
; maintained topic prefix relation is omitted.
...
counterexamples when each premise is removed. The file-fence theorem alone does not identify the final immutable link
or prove crash-image scanner provenance; the link step below addresses the
first of those obligations.

The next raw-byte step is
`fn-bs-k6-actual-record-linked-input-has-exact-frame`, also in
`books/byte-store-record-provenance.lisp`. Under a well-formed byte state,
`fn-bs-record-inputp` (which binds the frame to ACL2's candidate and the
transaction name to its sequence), a fresh staging key, and an absent final
name at the file-fence cut, the actual P-RECORD interpreter's pair 8
(`record-linked`) resolves that final name to the new inode, proves it is
fenced, and proves its durable octets equal the complete `frame`. The proof
joins the successful file-fence cut to the actual immutable-link step;
`tests/acl2/byte-store-record-provenance-tests.lisp` supplies a second-record
witness and separate failing examples for the four top-level premises.
The linked-cut theorem is a pre-crash view assertion. The next theorem
carries its bytes through a modeled crash image. General K0 remains
necessary to connect arbitrary served call traces to admissible
byte/kernel states and input premises.

The next K6 theorem, `fn-bs-k6-actual-linked-crash-decoder-reads-candidate`,
now carries that raw frame through every `fn-bs-crash-imagep` image of the
actual `record-linked` cut **whose final name survives**. It requires the
file-fence cut to have no earlier pending operation at that final name;
live absence alone is insufficient because a pending set/delete pair can
expose an unrelated older inode after a crash. The proof derives durable
absence from no prior operation plus live absence, proves the link is the
sole pending operation at that name, uses the crash-selector entry theorem
to recover the exact target, and uses the fenced-content theorem to keep
its complete octets. `fn-bs-record-of` on the recovered name therefore
equals the ACL2 candidate. The test book includes a previous durable
article, surviving/dropped link images, a corrupted non-crash image, and a
modeled prior set/delete counterexample. The same raw-frame/decoder conclusion holds at the actual pair-10
`record-attempted` cut: `fn-bs-k6-actual-attempted-cut-keeps-linked-byte-state`
proves that the link callback and named crash cut do not change the byte
state. This is the decoder at the recovered final name, not yet a whole-list
theorem about
`fn-bs-scan-records (fn-bs-scan-store image)`. That list additionally needs
the namespace contiguity, sequence and retained-prefix bridge, while
general K0 must establish the input relation for every served P-RECORD call.
`fn-bs-k6-related-attempted-surviving-scan-source-is-exact-frame` now
derives both earlier-operation absence and final-name absence for an actual
P-RECORD run starting from a byte/kernel-related `:record-staged` input. The
relation rules out a pending transaction-directory operation and makes the
candidate sequence the next index of the contiguous durable namespace; the
interpreted create/write/file-fence prefix leaves that directory and its
durable namespace untouched. `fn-bs-record-inputp` uses the typed
...
; shadow.  The issued-link premise is necessary: fn-bs-store-relation plus
; :record-attempted alone admits a logical :ok callback on a pre-link byte
; cut.  The actual P-RECORD trace has the pending link at this cut.
...
K1 to K4 are proved as of 2026-09-21 (lane `w11/k1-scan`); K5's stated
scan-record formula is proved under the byte/kernel relation. K8's corrected
issued-link post-fence crash-image scan formula is proved in
`books/byte-store-record-fence.lisp`. K6 exact raw-frame provenance and
general K0 are open. K0 must still establish that the real program's
`record-attempted` cut has issued the link before the successful barrier.
...
A positive GROUND witness for K1 -- `fn-bs-scan-okp` asserted TRUE of a
concrete image -- does not exist while §3.1's three seams are constrained
functions, because an `assert-event` evaluates and `fn-bs-config-okp`,
`fn-bs-frontier-decode` and `fn-bs-txn-name` have nothing to evaluate.
`tests/acl2/byte-store-scan-tests.lisp` says so in its header and carries
instead one concrete violating value per hypothesis of K1, reached through
the scan's first test, the config entry, which no constrained function
precedes. **Packet P4 is what makes the positive witness possible**, and
that is one more reason to take it: it is not only the twin the assurance
rules forbid, it is what keeps the keystone's teeth one-sided.
