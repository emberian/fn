...
own crash keystones K6 and K8, and general K0, are open; K5's stated
scan-record formula is proved under the byte/kernel relation below. The K8
fence now has an exact durable-record projection under the issued-link
premise; its universal crash-image scan conclusion remains open.
...
| K5-K8 | **K5's stated scan-record formula proved** in `books/byte-store-stable-prefix.lisp`: under the existing byte/kernel relation and a modeled crash image, the scanner retains the decoded durable list as a prefix and can add at most one record. Its test book reaches a second `record-linked` cut after an earlier transaction is durable, observes both absent/present images, and shows the bound fails separately when either relation or crash-image premise is dropped. The formula does not prove that the host establishes the relation at every cut (K0), nor independently qualify physical inode/barrier behavior. **K7 publication uncertainty proved for four native authority boundaries** in `books/byte-store-fault-keystones.lisp`: issued record link, issued allocator rename, transaction-directory barrier error and root-directory barrier error enter the recovery fence at the composed `fn-sn-io` subject. **K8's exact post-fence durable record list is proved for a related `:record-attempted` state with an issued pending link** in `books/byte-store-record-fence.lisp`; the result is the old durable list followed by the candidate, with an actual two-record P-RECORD witness. A negative fixture shows phase+relation alone admits an unissued-link pair. The remaining K8 step is to carry the byte/kernel relation through the fence and force every crash image's scan to equal that exact list. **K6's exact written-frame provenance, that K8 scan step, and general K0 remain open** (P3). |
