...
;;
;;   Lane p10-k0-b adds, under the same hypotheses:
...
(defmacro fn-bs-k0-frontier-rename-cut (name k kernel)
  `(defthm ,name
     (implies (and (fn-bs-store-relation bs ks)
                   (fn-bs-frontier-inputp ks stage octets)
                   (not (fn-bs-lookup bs :staging stage)))
              (let ((p (nth ,k (fn-bs-run bs ks (fn-bs-frontier-program stage octets) nil groups capacity))))
                (fn-bs-store-relation (car p) (cdr p))))
     :rule-classes nil
     :hints (("Goal" :do-not-induct t
...
(fn-bs-k0-frontier-rename-cut fn-bs-k0-frontier-replaced-cut-relation 9
  (cdr (nth 6 (fn-bs-run bs ks (fn-bs-frontier-program stage octets) nil groups capacity))))
(fn-bs-k0-frontier-rename-cut fn-bs-k0-frontier-attempted-cut-relation 11
  (fn-sf-frontier-replace-result
   (cdr (nth 6 (fn-bs-run bs ks (fn-bs-frontier-program stage octets) nil groups capacity))) :ok))
...
(defthm fn-bs-k0-frontier-replace-error-kernel-fields
  ...
  :hints (("Goal" :use ((:instance fn-sf-frontier-replace-result-preserves-state (s k6) (result :error)))
           :in-theory (e/d (fn-sf-frontier-replace-result)
                           (fn-sf-statep fn-sf-frontier-replace-result-preserves-state)))))
(defmacro fn-bs-k0-frontier-rename-arm (name k kernel)
  `(defthm ,name
     (implies (and (fn-bs-store-relation bs ks)
                   (fn-bs-frontier-inputp ks stage octets)
                   (not (fn-bs-lookup bs :staging stage)))
              (let ((p (nth ,k (fn-bs-run bs ks (fn-bs-frontier-program stage octets) nil groups capacity))))
                (fn-bs-store-relation (car p) ,kernel)))
     :rule-classes nil
     :hints (("Goal" :do-not-induct t
              :use (fn-bs-k0-frontier-observation-pair-facts
                    fn-bs-k0-frontier-file-observation-establishes-relation
              ...
              :in-theory (e/d (fn-bs-frontier-inputp fn-bs-rename fn-bs-replay-visiblep
                              ...
                              (fn-bs-run fn-bs-frontier-program fn-bs-store-relation fn-sf-statep
                               ...
                               fn-bs-durable-content fn-bs-lookup fn-bs-fencedp fn-bs-inop
                               fn-bs-make fn-bs-namep fn-sf-frontier-replace-result fn-sf-frontier-replace-result-preserves-state))))))
(fn-bs-k0-frontier-rename-arm fn-bs-k0-frontier-issued-rename-error-arm-relation 9
  (fn-sf-frontier-replace-result
   (cdr (nth 6 (fn-bs-run bs ks (fn-bs-frontier-program stage octets) nil groups capacity))) :error))
