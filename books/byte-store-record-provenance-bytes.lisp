; K6 and K0 byte-level provenance of the P-RECORD and P-FRONTIER programs:
; the byte-store part of books/byte-store-record-provenance, which includes
; this book, then its Store-node and configured-owner parts.
...
(defthm fn-bs-k6-empty-true-list
         (implies (true-listp xs)
...
(defthm fn-bs-k0-inode-list-knownp-append
...
(defthm fn-bs-k0-file-cut-keeps-other-fenced
  (implies (and (fn-bs-statep bs) (fn-bs-namep stage)
                (not (fn-bs-lookup bs :staging stage))
                (true-listp frame)
                (not (equal other (fn-bs-next-ino bs)))
...
(defthm fn-bs-k0-all-fencedp-append
...
; Proof steps the Store-node and owner parts reuse; exported disabled so
; that books including byte-store-record-provenance see no new enabled rule.
(in-theory (disable fn-bs-k0-pending-entry-targets-of-append
                    fn-bs-k6-created-stage-lookup-without-namep
