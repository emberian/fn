...
(defun fn-ncfg-string-value (value default bound requiredp)
  ...
  (cond ((not (natp bound)) :bad)
        ((null value) (if requiredp :bad default))
        ((and (true-listp value) (equal (len value) 2)
              (equal (car value) :string)
              (stringp (fn-ncfg-second value))
              (consp (fn-record-string-octets (fn-ncfg-second value)))
              (<= (len (fn-record-string-octets (fn-ncfg-second value))) bound))
...
(defun fn-native-config-load (octets)
  ...
  (if (and (fn-ncfg-ascii-octetsp octets) (<= (len octets) *fn-ncfg-max-octets*))
      (let ((pairs (fn-ncfg-parse-lines (fn-ncfg-lines octets) nil nil nil)))
        (if (or (equal pairs :bad) (< *fn-ncfg-max-lines* (len (fn-ncfg-lines octets))))
            (list :refused :syntax)
