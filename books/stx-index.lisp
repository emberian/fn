...
(local (defthm fn-stx-scan-of-bigger-lace
         ...
         :hints (("Goal" :induct (fn-lace-equivocator-scan rest lace p i)
                  :in-theory (enable (:d fn-lace-equivocator-scan))))))
...
(defun fn-stx-equivocation-record (held new)
  ...
  (list :equivocation
        ...
        (fn-stmt-id held) (fn-stmt-id new)))
...
(defun fn-stx-index-add1 (index s)
  ...
  (let* ((b (fn-stx-index-bindings index))
    ...
    (fn-stx-make-index
     ...
     (if (and prev (not (equal (cdr prev) s)))
...
; The one induction: the three queries and the two presence tests agree,
; conjunct by conjunct, over every store.
(defthm fn-stx-index-of-store-agrees
  (let ((index (fn-stx-index-of-store articles keyring))
        (lace (fn-stx-lace-of-store articles keyring)))
    (and (equal (fn-stx-index-lookup index id) (fn-lace-lookup lace id))
         (iff (fn-stx-alist-get id (fn-stx-index-bindings index))
              (member-equal id (fn-lace-ids lace)))
         (equal (fn-stx-index-slot-first index k)
                (fn-stx-lace-slot-first lace k))
         (iff (fn-stx-alist-get k (fn-stx-index-slots index))
              (fn-stx-lace-slot-first lace k))
  ...
  :hints (("Goal" :induct (fn-stx-index-of-store articles keyring)
           :in-theory (disable fn-stx-delta (:d fn-lace-slot-conflictp)))
          ("Subgoal *1/1" :use
           ((:instance fn-stx-equivocatorp-of-one-more
                       (lace (fn-stx-lace-of-store (cdr articles) keyring))
                       (s (car (fn-stx-delta (fn-article-payload (car articles))
                                             keyring))))
...
(defthm fn-stx-index-agrees-with-lace
  ...
  :rule-classes nil
  :hints (("Goal" :use ((:instance fn-stx-index-of-store-agrees
                                   (articles (fn-stx-store node))))
           :in-theory (disable fn-stx-index-of-store fn-stx-lace-of-store
