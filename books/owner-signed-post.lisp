...
(defthm fn-osp-authorized-event-binds-the-post
  (let ((e (fn-pa-authorized-event
        ...
        (plan (fn-pa-current-plan received snapshots)))
    (implies e
  ...
  :hints (("Goal"
           :use (fn-pa-authorized-event-requires-current-plan-by-definition
                 (:instance fn-osp-carried-event-base-shape
                  (keyring-generation (nth 6 (fn-pa-current-plan received snapshots)))
                  (enrolled-snapshot (fn-stxk-snapshot
                                      (nth 5 (fn-pa-current-plan received snapshots))))
                  (source (nth 1 (fn-pa-current-plan received snapshots)))
                  (principal (nth 2 (fn-pa-current-plan received snapshots)))
                  (keys (nth 3 (fn-pa-current-plan received snapshots)))
                  (signatures (nth 4 (fn-pa-current-plan received snapshots)))
...
(defthm fn-osp-plan-refusal-is-a-served-reason
  (implies (equal (car (fn-pa-current-plan received snapshots)) :refused)
           (and (member-equal (cadr (fn-pa-current-plan received snapshots))
