...
(defun fn-served-nntp-run (session archive events)
  (declare (xargs :guard t))
  (if (consp events)
      (let* ((here (fn-nntp-step session archive (car events)))
...
(local
 (defthm fn-served-nntp-run-of-no-events
   (implies (not (consp events))
            (equal (fn-served-nntp-run session archive events)
                   (fn-nntp-make-result session nil)))))

(local
 (defthm fn-served-nntp-run-is-consp
   (consp (fn-served-nntp-run session archive events))
   :rule-classes :type-prescription
   :hints (("Goal" :in-theory (e/d (fn-nntp-make-result)
                                   (fn-nntp-step))))))

(defthm fn-served-nntp-run-session-is-nntp-run-session
  (equal (fn-nntp-result-session (fn-served-nntp-run session archive events))
         (fn-nntp-run-session session archive events))
  :hints (("Goal" :induct (fn-served-nntp-run session archive events)
...
(defthm fn-served-nntp-run-preserves-consistent-session
  (implies (fn-nntp-session-consistentp session archive)
           (fn-nntp-session-consistentp
            (fn-nntp-result-session (fn-served-nntp-run session archive events))
            archive))
  :hints (("Goal"
...
(defthm fn-served-nntp-run-effects-are-typed
  (implies (fn-nntp-session-consistentp session archive)
           (fn-nntp-effectsp
            (fn-nntp-result-effects (fn-served-nntp-run session archive events))))
  :hints (("Goal" :induct (fn-served-nntp-run session archive events)
...
(defun fn-served-step (conn octets)
  ...
  (let* ((wire (fn-served-conn-wire conn))
         ...
         (run (fn-served-nntp-run (fn-served-conn-session conn) archive
...
(defthm fn-served-step-effects-are-typed
  (implies (fn-served-connp conn)
           (fn-nntp-effectsp (fn-served-result-effects (fn-served-step conn octets))))
  :hints (("Goal"
...
(defthm fn-served-run-effects-are-typed
  (implies (and (fn-served-connp conn)
                (fn-served-chunk-listp chunks))
           (fn-nntp-effectsp (fn-served-result-effects (fn-served-run conn chunks))))
  :hints (("Goal" :induct (fn-served-run conn chunks)
           :in-theory (disable fn-served-step fn-served-connp
                               fn-nntp-effectp))))
...
(local
 (defthm fn-served-nntp-run-session-of-append
  (equal (fn-nntp-run-session session archive (append left right))
         (fn-nntp-run-session (fn-nntp-run-session session archive left)
                              archive right))
  :hints (("Goal" :induct (fn-nntp-run-session session archive left)
           :in-theory (disable fn-nntp-step fn-nntp-session-consistentp
                               fn-nntp-projectionp)))))

(defthm fn-served-nntp-run-of-append
  (equal (fn-served-nntp-run session archive (append left right))
         (fn-nntp-make-result
          (fn-nntp-result-session
           (fn-served-nntp-run
            (fn-nntp-result-session (fn-served-nntp-run session archive left))
            archive right))
          (append
           (fn-nntp-result-effects (fn-served-nntp-run session archive left))
           (fn-nntp-result-effects
            (fn-served-nntp-run
             (fn-nntp-result-session (fn-served-nntp-run session archive left))
             archive right)))))
  :hints (("Goal" :induct (fn-served-nntp-run session archive left)
           :in-theory (disable fn-nntp-step fn-nntp-session-consistentp
                               fn-nntp-projectionp
