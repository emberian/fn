...
| C2-11 / Terra | Local web reader/composer and agent/operator client over the same owner API; C1 service contract, C2 signing API for signed actions | Useful threading including missing/cyclic references, drafts, explicit post/queued/received states, provenance and capacity/recovery status. No frontend acceptance logic or claimed human-read receipt. Bound rendering, escape untrusted HTML/content, disable active article content; test cross-origin/session boundaries where applicable. UI placement is provisional pending user preference. `HST-001/002`, `NNT-005/006`, `SCN-012/014/015`. |
...
| --- | --- | --- |
| D01 | Exact source bytes plus separate injection/trace projections, with conflict/legacy examples; user question pending | Local prototype source/provenance algebra and legacy injection tests; no permanent native signing grammar |
...
| D17 | Bring a small local web reader/composer alongside existing-reader interoperability; user question pending | API/thread/provenance design; UI lane is provisional and can yield its slot to operator tools |
...
D02/D03/D04 stay selected: native author signatures plus legacy provenance;
