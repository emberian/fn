...
| --- | --- |
| Books read | 226 |
| Certification roots in the Makefile | 225 |
| Books inside the root closure | 225 |
| `defthm` and `defthmd` events | 4233 |
| `defun` events | 3069 |
| Functions with verified guards | 1265 |
| Functions declared `:verify-guards nil` and never verified | 141 |
| Functions left at the default with an explicit guard | 1212 |
| Functions left at the default with no guard | 451 |
| `assert-event` checks | 4059 |
| `must-fail` checks | 59 |
...
| `fn-cpc-uint32p-bridge` | `books/checkpoint-codec.lisp` | 1058 | recognizer-body-conclusion: the conclusion is the body of the hypothesis fn-record-uint32p |
