...
| --- | --- |
| Books read | 181 |
| Certification roots in the Makefile | 180 |
| Books inside the root closure | 180 |
| `defthm` and `defthmd` events | 2850 |
| `defun` events | 1993 |
| Functions with verified guards | 942 |
| Functions declared `:verify-guards nil` and never verified | 53 |
| Functions left at the default with an explicit guard | 568 |
| Functions left at the default with no guard | 430 |
| `assert-event` checks | 3036 |
| `must-fail` checks | 71 |
