...
The current batch adds no product protocol or new persistent record kind.

| Boundary | Evidence at `ff381d1` | Exact next closure | Owner/status |
| --- | --- | --- | --- |
| Sender work → article/archive node | Typed finite state/node and transport receipt preservation; outbound rechecks actual binding | A joint invariant for durable and pending work, preserved by actual steps and arbitrary finite traces | Sol, active |
| Receiver context → recovered Store → receipt | Executable acceptance/decision/replay; concrete assertions; successful actual exchange | General preservation tying retained contexts to authoritative ready Store records and pending/committed receipts to their contexts | Astra, active |
| Receiver host → durable recovery | Seven exception/reopen cases, separate journal tests, actual BP process restart | Deterministic child-process deaths at FNBI, Store/context, context/intent, intent/decision and decision/delete boundaries; exact content and obligation checks on reopen | Terra, active |
| Logical BP functions → raw execution | Interpreted ACL2 host; some base functions have verified guards | Complete guard closure for the selected BP call graph before claiming raw execution; report actual ACL2 status separately | Follow-on, open |
...
After this bounded batch, select the next complete item from BP guards/journal
refinement, native injection/signature/authority, physical storage accounting and
platform qualification. Multi-relay/contact-plan work must carry its own failure
and progress obligations when it is added. Checkpoint/index logical results and
