#!/bin/sh
# bp-lifecycle-3: at lane commit REV (unpacked into $S/tREV), install the dtn
...
S=/tank/fn/scratch/bp-lifecycle-3; T=$S/t$REV; L=$S/logs-$REV
...
for M in $MODS; do
  if [ $M = test_bp_fragment_node_native ]; then export FN_NATIVE_DEVELOPER_HOST=$B/fn-host-dtn-developer; else export FN_NATIVE_DEVELOPER_HOST=$B/fn-host-developer; fi
  s=$(date +%s)
