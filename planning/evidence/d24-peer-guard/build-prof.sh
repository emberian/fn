#!/bin/sh
# usage: build-prof.sh TREE -- the developer image with the sprof entry, from TREE's certified books
set -eu
ROOT=$1
cd "$ROOT"
export FN_OPENSSL_PREFIX=/tank/fn/toolchains/openssl-3.5.8
awk '/^\(defttag nil\)/{while((getline l < "/tank/fn/scratch/d24-peer/sprof-hook.lisp")>0) print l} {print}' host/native/build.lisp > build/prof-build.lisp
