...
# against IMAGE-TREE's developer image, N = 16, 50, 120.
T=$1; L=$2; S=/tank/fn/scratch/d24-peer; TOOL=$S/after-tree/tools/msgid_measure.py
...
for n in 16 50 120; do
  rm -rf $S/$L-n$n
  python3 $TOOL --image $T/build/fn-host-developer --work $S/$L-n$n --articles $n --samples 16 --json $S/$L-n$n.json > $S/$L-n$n.log 2>&1 </dev/null
