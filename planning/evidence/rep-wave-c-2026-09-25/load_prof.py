#!/usr/bin/env python3
"""Scratch (rep-wave-c lane): a profile-aware load of a native image.

    load_prof.py TOOLS --image IMG --work DIR --articles N --payload-octets P \
        [--profile default] [--stats K] [--count COUNT_LISP] --json OUT

The store is initialised through the operator verb under the named profile
(`operator CONFIG init --profile PROFILE GROUP`; `default` is the D27 default
profile: the transaction ceiling and 16 MiB articles), so N = 10,000 and
32 KiB are admitted.  N articles of about P octets go over one NNTP
connection with every POST timed; RSS is read at a quarter and at the end.
With --stats K the image's profiling thread (FN_SPROF_DIR) covers the last K
POSTs: CPU samples (flat.txt, graph.txt), or with --count the call counts of
the wrapped functions (FN_PROF_LOAD=COUNT_LISP, FN_PROF_COUNT=1: counts.txt).
"""
...
if a.stats:
    ...
    if a.count:
        env["FN_PROF_LOAD"] = str(Path(a.count).resolve()); env["FN_PROF_COUNT"] = "1"
port = m.free_port()
store, control, config = work / "store", work / "control.sock", work / "fn.toml"
config.write_text('[store]\npath = "%s"\n[listener]\nhost = "127.0.0.1"\nport = %d\n[control]\npath = "%s"\n' % (store, port, control), encoding="ascii")
init = subprocess.run([str(image), "--fn", "operator", str(config), "init", "--profile", a.profile, "fn.test"],
                      env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
(work / "init.out").write_bytes(init.stdout)
if init.returncode != 0:
    raise SystemExit("init failed rc=%d: %r" % (init.returncode, init.stdout[-400:]))
...
out = {"image": str(image), "launcher_sha256": m.digest(image), "core_sha256": m.digest(str(image) + ".core"),
       "profile": a.profile, "articles": a.articles, "payload_octets": a.payload_octets,
       "article_octets": len(article(0)), "stats": a.stats, "count": bool(a.count),
       "init_out": init.stdout.decode("utf-8", "replace")[-300:]}
try:
    ...
    if a.stats:
        for name in ("counts.txt", "error.txt"):
            if (sp / name).exists():
                out[name.split(".")[0]] = (sp / name).read_text()
        if "counts" in out:
            out["counts_per_post"] = {l.split()[0]: int(l.split()[1]) / a.stats for l in out["counts"].splitlines() if l.strip()}
finally:
    proc.terminate()
...
print("profile", a.profile, "articles", a.articles, "payload", a.payload_octets, "load_s", round(out.get("load_seconds", 0), 3),
      ...
      "counts_per_post", out.get("counts_per_post"), "error", out.get("error"))
