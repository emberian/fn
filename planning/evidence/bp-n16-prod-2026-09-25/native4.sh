#!/bin/sh
# bp-n16-prod-2 (run 4, fragmentation): at the lane tree, certify dtn and default roots from the
...
set -u
S=/tank/fn/scratch/bp-n16-prod; T=$S/ta4d95797; L=$S/logs4
...
export FN_NATIVE_TEST_DIAGNOSTIC_DIR=$S/diag4; mkdir -p $S/diag4
systemd-run --user --scope -q -p MemoryMax=24G timeout 900 python3 -m unittest -v tests.test_bp_node_native.NativeBpNodeTests.test_request_above_peer_mru_is_fragmented_and_reassembled > $L/fragment-case.log 2>&1 < /dev/null
echo "fragment case rc=$? $(tail -n 3 $L/fragment-case.log | grep -E '^(OK|FAILED)')"; sha256sum $L/fragment-case.log
tail -n 3 $L/fragment-case.log | grep -q '^OK' || { tail -40 $L/fragment-case.log; echo STOPPED; exit 1; }
s=$(date +%s)
