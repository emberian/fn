...
S=/tank/fn/scratch/harness-repair
T=${NS_TREE:-$S/tree}
L=$S/logs
mkdir -p "$L"
cd "$T"
...
# harness-repair: the named modules through tools/test_budget.py (180 s a module, 20 s a test).
cd "$T"
{ echo "# harness-repair native in $T image bbf52159 start $(date -u +%FT%TZ)"; timeout 3000 python3 tools/test_budget.py "$@" --logs $S/budget-logs --json $S/budget-report.json; echo "# rc=$? end $(date -u +%FT%TZ)"; } > $L/test_budget.log 2>&1
