...
Source: `implement/bp-foundation` `6e80ccaf` from `df5097b6`.
...
its status distinguishes pending from uncertain. `fn-bpnf-wait` names an
...
`run-20260923T142534Z-64cf`, manifest
`planning/evidence/manifests/certify-20260923T142536Z-3851584.json`.
ACL2 8.7 on SBCL 2.6.8, toolchain identity
`d5f2b9f0d2cf68c6074ea7046f4bd2e560d2984fe22d7e03f93045975ac889f0`.
The manifest reports both requested roots passed at the recorded source
digests, with 26 dependencies installed from the matching cache. The earlier
...
reduced the bridge to a 0.01-second proof in `proof_repl` before the final
certification. `green_check.py --changed-since df5097b6 --strict` reports
...
`(epoch, operation-id, :store, held, :pending/:uncertain)`, its `:persist`
effect and the matched durable installation. A2 needs an FNBS-specific
