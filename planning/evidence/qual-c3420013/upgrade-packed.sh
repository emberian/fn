...
W=$S/upgrade-packed; rm -rf $W; mkdir -p $W/orig
...
stop; "$OLD" --fn operator $W/orig/fn.toml store compact; echo "compact (4eca4148) rc=$?"; tree $W/orig; echo "pack files: $(ls $W/orig/store | grep -iv -E "^(transactions|staging|config|config.json|writer.lock|allocation-frontier.json)$" | tr "\n" " ")"
