...
freeze=root/'build/freeze'; freeze.mkdir(parents=True,exist_ok=True)
...
env=dict(os.environ,FN_ACL2=str(wrapper),ACL2_CUSTOMIZATION='NONE',ACL2_BOOK_HASH_ALISTP='NIL',FN_OPENSSL_PREFIX=str(image/'openssl'),FN_NATIVE_HOST=str(image/'fn-host'),FN_NATIVE_DEVELOPER_HOST=str(image/'fn-host-developer'),FN_NATIVE_DTN_DEVELOPER_HOST=str(image/'fn-host-dtn-developer'),FN_NATIVE_CRASH_HOST=str(image/'fn-host-developer'),FN_NATIVE_IMAGE_SOURCE_SHA=rev,FN_PRE_T2_NATIVE_DEVELOPER_HOST=str(old),FN_T2_NATIVE_DEVELOPER_HOST=str(image/'fn-host-developer'),FN_T2B_NATIVE_DEVELOPER_HOST=str(image/'fn-host-developer'),FN_RUN_HYBRID_E2E='1',FN_PRE_T2_IMAGE=str(old),FN_T2_IMAGE=str(image/'fn-host-developer'),FN_T2_PROBE_ROOT=str(freeze/'migration-stores'))
