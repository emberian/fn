...
(Copied to dev from spike/mega b4d0184c by the night deputy, because dev books cite it; the spike is the specification, never merged into dev.)

Lane `spike/operator` on `spike/mega` (D28). One `fn` command
(`packaging/fn` on spike/mega, bash, host-side) over the native
...
Operator documentation: docs/operator.md on branch spike/mega, section "Operating a native node with fn (spike, D28)".
