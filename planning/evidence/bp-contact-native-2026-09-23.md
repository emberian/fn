...
The native caller `host/native/bp-contact.lisp` calls
`fn-bpsc-relative-window`, `fn-bpsc-contact-decision`, and
`fn-bpsc-contact-event` from
...
window emits a close event. CLI window arguments are delays from this tick's
observation, never absolute monotonic timestamps shared across processes or
reboot. The sender's TCPCL result stays accepted,
...
invalid-event equations are direct by-definition facts connecting that
decision to the host's `:contact` event, not separate keystones.
`tests/acl2/bp-contact-service-tests.lisp` provides a reachable open contact,
wrong-peer, absent-ready-peer, outside-window, malformed-window, relative
window, and 64-bit overflow witnesses.
...
tests/acl2/bp-contact-service-tests`. Final run `run-20260923T185009Z-900b`
passed; the [manifest](manifests/certify-20260923T185010Z-2954887.json) records
ACL2 8.7 / SBCL 2.6.8, the source digests, 14 cached dependencies, two
certified roots, 1.539 and 1.129 seconds per root, and 2.674 seconds total
certificate wall time. An earlier bounded run found an opening-theorem rule
class error; it was fixed before the successful runs. This final run also
re-certified the scoped clock-guard hint and relative-window addition.
