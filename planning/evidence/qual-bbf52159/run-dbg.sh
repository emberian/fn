...
for m in "$@"; do
  ...
  s=$(date +%s.%N)
  timeout 3600 python3 -c "import tools.run_store as r; o=r.acl2_boolean; r.acl2_boolean=lambda v: (print('BRIDGE-VALUE', repr(v)[:2000]), o(v))[1]; import unittest,sys; sys.argv=['x','-v','tests.$m']; unittest.main(module=None)" >> "$log" 2>&1
  rc=$?
  e=$(date +%s.%N)
