#!/bin/sh
# qual-bbf52159 harvest: logs, judged tables, scripts; no store copies of the live node, no keys.
set -u
S=/tank/fn/scratch/qual-bbf52159
H=$S/harvest; rm -rf $H; mkdir -p $H
C=$S/campaign
cd $S
cp campaign.sh chainA.sh chainB.sh chainC.sh chainD.sh budget.sh compare.py run-dbg.sh run.sh setup.sh reloc.sh labs.sh marker_cuts.py d25b_matrix.py judge.py faults.py checkpoint_judge.py web-manual.py upgrade_live.py probe_budget.py probe_cost.py probe_cleanup_prod.py harvest.sh $H/ 2>/dev/null
cp campaign.out chainA.out chainB.out chainC.out chainD.out probe-cleanup-prod.log budget-report.json image-check.log image-full.sha256 pylib.txt web-manual.log ss-before.txt $H/ 2>/dev/null
cp upgrade-live.log upgrade-live/pristine-store.sha256 probe-budget.log $H/ 2>/dev/null
...
( cd ~/fn-deploy/native-bbf52159-95279f8 && tar -czf $H/matrix-node-logs.tgz $(ls -d a/*.log a/fn.toml b/*.log b/fn.toml gate-run/*.log 2>/dev/null) 2>/dev/null )
