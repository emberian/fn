#!/usr/bin/env python3
"""qual-bbf52159 upgrade rehearsal on COPIES of the live node's store and fn.toml.
...
S = Path("/tank/fn/scratch/qual-bbf52159/upgrade-live")
T = Path("/tank/fn/scratch/qual-bbf52159/tree")
I = Path("/tank/fn/gates/qual-bbf52159-20260925/build/images/bbf52159dcab19228bd6cd0b855b99dd6d68758d")
LIVE = Path("/tank/fn/node")
OPENSSL = "/tank/fn/scratch/qual-bbf52159/bin/test-openssl"
LOGIN, SECRET = "qual", "qual-bbf52159-rehearsal"
P = bytes([85]) * 32
...
def main():
    ...
    rc, out = sh("install-native bbf52159", ["env", "PREFIX=%s" % (S / "fn-bbf52159"),
                                             "FN_NATIVE_HOST=%s" % (I / "fn-host"),
                                             "FN_NATIVE_SOURCE_REVISION=bbf52159dcab19228bd6cd0b855b99dd6d68758d",
    ...
    sh("bbf52159 status offline on the format-7 store", [NEW, "operator", cfg, "status"], expect=0)
    sh("bbf52159 store needs-upgrade (format 7)", [NEW, "operator", cfg, "store", "needs-upgrade"])

    # 2. the new owner on the format-7 store (deploy step 1), live status ----
    owner = start([NEW], cfg, "bbf5-format7")
    sh("bbf52159 status live (owner running, format 7)", [NEW, "operator", cfg, "status"], expect=0)
    c = Nntp(port, cert, login=False)
    say("  greeting %s / STARTTLS %s" % (c.greeting, c.starttls))
    c.cmd("GROUP fn.agents")
    c.close()
    stop(owner, "bbf5-format7")
    sh("bbf52159 status offline after the first owner", [NEW, "operator", cfg, "status"], expect=0)
    ...
    sh("same: bbf52159 status (the requirement is gone?)", [NEW, "operator", cfg_r3, "status"])
    os.rename(r3 / "store" / "committed-history.json", S / "r3-marker")
    sh("same, marker removed: bbf52159 recover (legacy open admits?)", [NEW, "operator", cfg_r3, "recover"])
    ...
    owner = start([NEW], cfg, "bbf5-required")
    ...
    tin_id = "<qual-bbf5-tin@example.invalid>"
    ...
    signed_id = "<qual-bbf5-signed@example.invalid>"
    ...
    cancel_id = "<qual-bbf5-cancel@example.invalid>"
    ...
    sh("A3: control list (live, after the grant)", [NEW, "operator", cfg, "control", "list"], expect=0)
    succ_id = "<qual-bbf5-succession@example.invalid>"
    ...
    c.post(carrier(kset["old"], header("<qual-bbf5-oldkey@example.invalid>", "fn.agents", "old key") + b"x\r\n", "oldkey"))
    c.post(carrier(kset["new"], header("<qual-bbf5-newkey@example.invalid>", "fn.agents", "new key") + b"x\r\n", "newkey"))
    c.close()
    stop(owner, "bbf5-required")
    log = (work / "log" / "fn.log")
    if log.exists():
    ...
    owner = start([NEW], cfg, "bbf5-required-restart")
    c = Nntp(port, cert)
    c.cmd("GROUP fn.agents")
    for m in (tin_id, signed_id, cancel_id, "<qual-bbf5-newkey@example.invalid>"):
        c.article(m)
    c.close()
    stop(owner, "bbf5-required-restart")
    sh("recover after restart", [NEW, "operator", cfg, "recover"], expect=0)
    sh("A3: control list (offline)", [NEW, "operator", cfg, "control", "list"], expect=0)

    # 7. invite / accept / confirm between two copies, and a pull ------------
    # A = the migrated live copy (format 8, required, auth required, TLS).
    # B = a second copy of the live store, format 8, with a plain loopback
    # fn.toml (no auth) and its own path identity.
    bdir = S / "peer-b"
    copytree(pristine, bdir)
    bport = free_port()
    bcfg = bdir / "fn.toml"
    bcfg.write_text('[store]\npath = "%s"\n[listener]\nhost = "127.0.0.1"\nport = %d\n[control]\npath = "%s"\n[log]\npath = "%s"\n'
                    % (bdir / "store", bport, bdir / "control.sock", bdir / "fn.log"))
    sh("B: store upgrade-profile (7 to 8)", [NEW, "operator", bcfg, "store", "upgrade-profile"], expect=0)
    sh("B: policy set path-identity b.qual.invalid", [NEW, "operator", bcfg, "policy", "set", "path-identity", "b.qual.invalid"], expect=0)
    sh("A: policy show", [NEW, "operator", cfg, "policy", "show"])
    def keygen(d):
        d.mkdir(parents=True)
        der, pub = d / "ed-private.der", d / "ed-public.der"
        subprocess.run([OPENSSL, "genpkey", "-algorithm", "ED25519", "-outform", "DER", "-out", str(der)], check=True)
        subprocess.run([OPENSSL, "pkey", "-inform", "DER", "-in", str(der), "-pubout", "-outform", "DER", "-out", str(pub)], check=True)
        seed, public = der.read_bytes()[-32:], pub.read_bytes()[-32:]
        (d / "ed-public.bin").write_bytes(public)
        (d / "ed-secret.bin").write_bytes(seed + public)
        der.unlink(); pub.unlink()
        subprocess.run([OPENSSL, "genpkey", "-algorithm", "ML-DSA-65", "-out", str(d / "ml-private.pem")], check=True)
        subprocess.run([OPENSSL, "pkey", "-in", str(d / "ml-private.pem"), "-pubout", "-out", str(d / "ml-public.pem")], check=True)
        return d
    ka, kb = keygen(S / "peer-keys-a"), keygen(S / "peer-keys-b")
    sh("A: peer genesis", [NEW, "operator", cfg, "peer", "genesis", ka], expect=0)
    sh("B: peer genesis", [NEW, "operator", bcfg, "peer", "genesis", kb], expect=0)
    pa, pb = (ka / "principal.bin").read_bytes().hex(), (kb / "principal.bin").read_bytes().hex()
    say("  principals A=%s B=%s" % (pa, pb))
    oa = start([NEW], cfg, "bbf5-peer-A")
    ob = start([NEW], bcfg, "bbf5-peer-B")
    inv, acc = S / "peer-inv", S / "peer-acc"
    sh("A: peer invite nodeB", [NEW, "operator", cfg, "peer", "invite", "nodeB", "fn.*", "127.0.0.1", str(bport),
                                "hbox.ember.software", ka, inv], expect=0)
    sh("B: peer accept", [NEW, "operator", bcfg, "peer", "accept", inv, kb, "b.qual.invalid", "-", acc], expect=0)
    sh("A: peer confirm", [NEW, "operator", cfg, "peer", "confirm", acc], expect=0)
    sh("A: peer confirm again (expected refusal already-confirmed)", [NEW, "operator", cfg, "peer", "confirm", acc], expect=1)
    sh("B: peer accept again (expected refusal already-enrolled)", [NEW, "operator", bcfg, "peer", "accept", inv, kb,
                                                                   "b.qual.invalid", "-", S / "peer-acc-again"], expect=1)
    sh("A: peer list (live)", [NEW, "operator", cfg, "peer", "list"])
    sh("B: peer list (live)", [NEW, "operator", bcfg, "peer", "list"])
    # B posts an article; A pulls it from B.
    pull_id = "<qual-bbf5-pull@example.invalid>"
    with socket.create_connection(("127.0.0.1", bport), 60) as raw:
        f = raw.makefile("rwb", buffering=0)
        f.readline(); f.write(b"POST\r\n"); r1 = f.readline()
        f.write(header(pull_id, "fn.agents", "qual pull from B") + b"pulled body\r\n.\r\n"); r2 = f.readline()
        say("  B POST -> %s / %s" % (r1.decode().strip(), r2.decode().strip()))
    stop(oa, "bbf5-peer-A")
    sh("A: peer add nodeBpull (inbound fn.*, outbound none)", [NEW, "operator", cfg, "peer", "add", "nodeBpull", "b.qual.invalid",
                                                              "127.0.0.1", str(bport), "fn.*", "-", "127.0.0.9", "true"], expect=0)
    sh("A: peer pull nodeBpull 2", [NEW, "operator", cfg, "peer", "pull", "nodeBpull", "2"], expect=0)
    oa = start([NEW], cfg, "bbf5-pull-A")
    got = "none"
    deadline = time.time() + 90
    while time.time() < deadline:
        c = Nntp(port, cert)
        rep = c.cmd("STAT " + pull_id, show=False)
        c.close()
        if rep.startswith("223"):
            got = rep
            break
        time.sleep(2)
    say("STEP A pulled B's article: %s" % got)
    sh("A: status live (after the pull)", [NEW, "operator", cfg, "status"], expect=0)
    stop(oa, "bbf5-pull-A")
    stop(ob, "bbf5-peer-B")
    for nm, st in (("A", store), ("B", bdir / "store")):
        sh("%s: hybrid-key-history" % nm, [NEW, "hybrid-key-history", st], expect=0)
    alog = work / "log" / "fn.log"
    if alog.exists():
        for line in alog.read_text("utf-8", "replace").splitlines():
            if "pull" in line.lower() or "peer" in line.lower():
                say("  A log| " + line[:240])
    sh("A: recover after the peering steps", [NEW, "operator", cfg, "recover"], expect=0)
