#!/bin/sh
# control-c1: build the developer image from the lane tree and run the native C1 cases.
set -u
T=$1
...
FN_RUN_HYBRID_E2E=1 FN_TEST_OPENSSL=$FN_OPENSSL_PREFIX/bin/openssl FN_NATIVE_HOST=$T/build/fn-host-developer python3 -m unittest -v tests.test_native_control_filing > build/lane/native-control.log 2>&1; echo "native rc=$?"
grep -E "NATIVE-CONTROL-WITNESS|^(OK|FAILED)|Error|ok$|FAIL" build/lane/native-control.log
