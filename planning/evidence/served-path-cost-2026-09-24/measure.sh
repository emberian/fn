...
T=$1; L=$2; S=/tank/fn/scratch/served-path-cost
...
for n in 16 50 120; do
  rm -rf $S/$L-n$n
  python3 $T/tools/msgid_measure.py --image $T/build/fn-host-developer --work $S/$L-n$n --articles $n --samples 16 --json $S/$L-n$n.json > $S/$L-n$n.log 2>&1 </dev/null
