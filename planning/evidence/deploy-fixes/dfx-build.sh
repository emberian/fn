#!/bin/sh
set -eu
ROOT=/tank/fn/scratch/deploy-fixes/tree
...
FN_ACL2=$ACL2 FN_NATIVE_PROFILE=production FN_NATIVE_BUILD=host/native/build.lisp FN_NATIVE_IMAGE=build/fn-host FN_NATIVE_LOG=build/freeze/native-build-production.log swarm-build sh tools/build_native_host.sh > build/freeze/prod.out 2>&1 || { tail -30 build/freeze/prod.out; tail -30 build/freeze/native-build-production.log; exit 1; }
...
grep -ci "invariant-risk" build/freeze/native-build-production.log build/freeze/native-build-developer.log || true
