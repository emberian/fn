#!/bin/sh
# control-c3d: build the developer image from the lane tree and run the native control filing cases (signed cancel filed; the served withdrawal, test_signed_withdrawal, test_two_node_withdrawal).
set -u
T=$1
...
FN_RUN_HYBRID_E2E=1 FN_NATIVE_HOST=$T/build/fn-host-developer python3 -m unittest -v tests.test_native_control_filing > build/lane/native-control-filing.log 2>&1; echo "native rc=$?"
grep -E "NATIVE-CONTROL-WITNESS|NATIVE-WITHDRAWAL-WITNESS|NATIVE-TWO-NODE-WITHDRAWAL-WITNESS|^(OK|FAILED)|Error|ok$|FAIL" build/lane/native-control-filing.log
sha256sum build/lane/native-control-filing.log
