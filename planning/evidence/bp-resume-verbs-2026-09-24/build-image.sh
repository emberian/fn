...
# The lane's third farm run left bp-node-progress-bridge and its test book red
# (fixed at this tree's revision); certify them here before validating.
FN_ACL2_TIMEOUT_SECONDS=300 FN_CERT_CACHE=$CACHE swarm-build python3 tools/certify_books.py --jobs 2 --timeout-seconds 300 books/bp-node-progress-bridge tests/acl2/bp-node-counterexamples-tests > build/lane/certify-bridge.txt 2>&1; echo "bridge certify rc=$?"; tail -2 build/lane/certify-bridge.txt
