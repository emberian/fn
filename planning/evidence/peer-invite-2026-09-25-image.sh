#!/bin/sh
# peer-invite: build the developer image from the lane tree on hbox and run
# the native peering cases (tests/test_native_peer_invite.py).  Run ON hbox:
#   sh peer-invite-2026-09-25-image.sh /tank/fn/scratch/peer-invite/tree
set -u
T=$1
...
FN_NATIVE_HOST=$T/build/fn-host-developer systemd-run --user --scope -q -p MemoryMax=24G python3 -m unittest -v tests.test_native_peer_invite > build/lane/native-peer-invite.log 2>&1; echo "native rc=$?"
grep -E "NATIVE-PEER-INVITE|^(OK|FAILED)|Error|ok$|FAIL" build/lane/native-peer-invite.log
sha256sum build/lane/native-peer-invite.log
