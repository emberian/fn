#!/bin/sh
# control-c3: build the developer image from the lane tree and run the native control filing cases (signed cancel filed in control.cancel).
set -u
T=$1
...
FN_NATIVE_HOST=$T/build/fn-host-developer python3 -m unittest -v tests.test_native_control_filing > build/lane/native-control-filing.log 2>&1; echo "native rc=$?"
grep -E "NATIVE-CONTROL-WITNESS|^(OK|FAILED)|Error|ok$|FAIL" build/lane/native-control-filing.log
