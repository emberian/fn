#!/usr/bin/env python3
"""qual-dfa810fc upgrade rehearsal on COPIES of the live node's store and fn.toml.
...
S = Path("/tank/fn/scratch/qual-dfa810fc/upgrade-live")
T = Path("/tank/fn/scratch/qual-dfa810fc/tree")
I = Path("/tank/fn/gates/qual-dfa810fc-20260926/build/images/dfa810fceabedde937ba7e4bfa46b43fd599ac62")
LIVE = Path("/tank/fn/node")
OPENSSL = "/tank/fn/scratch/qual-dfa810fc/bin/test-openssl"
LOGIN, SECRET = "qual", "qual-dfa810fc-rehearsal"
P = bytes([85]) * 32
...
# qual-dfa810fc: packaging/fn (every install's bin/fn) runs $FN_NATIVE_HOST when it is set,
# so an inherited FN_NATIVE_HOST would make "bbf52159's bin/fn" run the candidate image.
for _k in [k for k in ENV if k.startswith("FN_NATIVE_") or k in ("FN_OLD_IMAGE", "FN_FORMAT7_IMAGE")]:
    ENV.pop(_k)
started = []
...
def main():
    ...
    rc, out = sh("install-native dfa810fc", ["env", "PREFIX=%s" % (S / "fn-dfa810fc"),
                                             "FN_NATIVE_HOST=%s" % (I / "fn-host"),
                                             "FN_NATIVE_SOURCE_REVISION=dfa810fceabedde937ba7e4bfa46b43fd599ac62",
    ...
    rc, new_status = sh("dfa810fc status offline (per-article accounting)", [NEW, "operator", cfg, "status"], expect=0)
    ...
    for k in ("transactions", "articles", "transactions-used", "transactions-budget", "bytes-used", "history-bound",
        ...
        say("  1b %-22s bbf52159=%s dfa810fc=%s" % (k, fo.get(k), fn_.get(k)))
    try:
    ...
    for line in new_status.splitlines():
        if line.startswith(("maintenance-reserve", "headroom", "profile", "open")):
            say("  1b line| " + line)
    sh("dfa810fc health offline", [NEW, "operator", cfg, "health"])
    sh("dfa810fc store needs-upgrade (format 8 unmarked)", [NEW, "operator", cfg, "store", "needs-upgrade"])
    sh("dfa810fc store rollback-check KEPT format-7 config (in the live store)",
       [NEW, "operator", cfg, "store", "rollback-check", store / "config.json.format-7"])
    sh("dfa810fc store rollback-check --snapshot of itself (nothing lost)",
    ...
    ow = start([NEW], cfg_o0, "dfa8-open-only")
    stop(ow, "dfa8-open-only")
    ...
    sh("bbf52159 status after dfa810fc opened with no commit", [OLD, "operator", cfg_o0, "status"])
    ...
    owner = start([NEW], cfg, "dfa8-deploy")
    ...
    body1 = b"qual-dfa810fc protected ordinary use: first post\n"
    rc, out = client(port, cert, "post", "fn.agents", "--subject", "qual dfa8 ordinary", "--message-id",
                     "<qual-dfa8-ordinary@example.invalid>", "--draft", S / "draft-ordinary.json", stdin=body1)
    rc, out = client(port, cert, "show", "<qual-dfa8-ordinary@example.invalid>")
    rc, out = client(port, cert, "post", "fn.agents", "--subject", "Re: qual dfa8 ordinary", "--message-id",
                     "<qual-dfa8-reply@example.invalid>", "--references", "<qual-dfa8-ordinary@example.invalid>",
                     stdin=b"a reply over the same protected session kind\n")
    rc, out = client(port, cert, "read", "fn.agents", "--new")
    rc, out = client(port, cert, "post", "fn.agents", "--subject", "qual dfa8 after resume", "--message-id",
                     "<qual-dfa8-later@example.invalid>", stdin=b"posted after the watermark\n")
    rc, out = client(port, cert, "read", "fn.agents", "--new")
    ...
    tin_id = "<qual-dfa8-tin@example.invalid>"
    ...
    signed_id = "<qual-dfa8-signed@example.invalid>"
    ...
    qid = "<qual-dfa8-wrong-principal@example.invalid>"
    ...
    cancel_id = "<qual-dfa8-cancel@example.invalid>"
    ...
    sh("bbf52159 status on the store dfa810fc wrote (expected refusal)", [OLD, "operator", cfg_r0, "status"])
    sh("bbf52159 recover on the store dfa810fc wrote (expected refusal)", [OLD, "operator", cfg_r0, "recover"])
    sh("dfa810fc store rollback-check KEPT format-7 config after the writes",
       [NEW, "operator", cfg, "store", "rollback-check", store / "config.json.format-7"])
    rc, out = sh("dfa810fc store rollback-check --snapshot SNAPSHOT (the loss count)",
    ...
    o = start([NEW], cfg_mk, "dfa8-required")
    c = Nntp(mport, mk / "tls" / "cert.pem")
    c.cmd("GROUP fn.agents")
    c.post(header("<qual-dfa8-required@example.invalid>", "fn.agents", "qual rehearsal required") + b"after required\r\n")
    c.close()
    stop(o, "dfa8-required")
