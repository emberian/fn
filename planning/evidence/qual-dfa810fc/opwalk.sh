...
set -u
T=$1
B=/tank/fn/scratch/qual-dfa810fc/operator-walk
W=$B/run
P=$B/prefix
export FN_OPENSSL_PREFIX=/tank/fn/toolchains/openssl-3.5.8
export LD_LIBRARY_PATH=$FN_OPENSSL_PREFIX/lib
for u in a b a2 copy nostream; do systemctl --user stop qual-dfa810fc-ow-$u 2>/dev/null; systemctl --user reset-failed qual-dfa810fc-ow-$u 2>/dev/null; done
...
start() { # NAME : waits for the listener, not the socket file (a killed owner leaves a stale one)
  ...
  systemd-run --user --unit qual-dfa810fc-ow-$1 -p MemoryMax=24G --setenv FN_OPENSSL_PREFIX=$FN_OPENSSL_PREFIX \
...
stop() { systemctl --user stop qual-dfa810fc-ow-$1; say "   stop $1: $(systemctl --user is-active qual-dfa810fc-ow-$1)"; }
...
systemd-run --user --unit qual-dfa810fc-ow-nostream -p MemoryMax=1G python3 $T/planning/evidence/operator-walk-2026-09-26/nostream_peer.py 31603 > /dev/null 2>&1
sleep 1; start a
art 2; step post-a-2 0 $FN operator $A post --message-id '<walk-2@example.invalid>' --payload $W/p/2 --group local.test
sleep 40
journalctl --user -u qual-dfa810fc-ow-nostream --no-pager -o cat > $O/nostream.log 2>&1
...
systemctl --user stop qual-dfa810fc-ow-nostream
...
start a
systemctl --user kill -s KILL qual-dfa810fc-ow-a; sleep 2; say "   crash: SIGKILL, unit $(systemctl --user is-active qual-dfa810fc-ow-a)"
systemctl --user reset-failed qual-dfa810fc-ow-a 2>/dev/null
