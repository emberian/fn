#!/usr/bin/env python3
"""qual-dfa810fc rehearsal follow-up (after upgrade_live.py): the rollback to
bbf52159 WITHOUT a snapshot, which the rehearsal showed is possible on this
image (dfa810fc writes the frontier in the format-2 bytes for every txid below
2^32, fn-bs-frontier-encode-impl), exercised end to end on copies:
  (a) bbf52159's owner on the store dfa810fc wrote: serve what dfa810fc
      accepted, accept a new article; then dfa810fc reopens and serves it;
...
def serve_and_post(image, name, work, cfg, tag, read_ids):
    ...
    for mid in read_ids:
        c.cmd("STAT " + mid)
    msgid = "<qual-dfa8-%s@example.invalid>" % tag
...
def main():
    ...
    seen = ["<qual-dfa8-tin@example.invalid>", "<qual-dfa8-signed@example.invalid>",
            "<qual-dfa8-cancel@example.invalid>", "<qual-dfa8-wrong-principal@example.invalid>"]
    # (a) plain deployed store (unmarked), written by dfa810fc
    a = S / "rollback-binary"
    U.copytree(S / "w", a)
    cfg_a = U.config_for(a, U.free_port())
    U.sh("bbf52159 recover on the store dfa810fc wrote", [OLD, "operator", cfg_a, "recover"], expect=0)
    old_id = serve_and_post(OLD, "bbf5-on-dfa8-store", a, cfg_a, "by-bbf5", seen)
    U.sh("bbf52159 status after its own commit", [OLD, "operator", cfg_a, "status"], expect=0)
    U.sh("dfa810fc recover after bbf52159 wrote (roll forward again)", [NEW, "operator", cfg_a, "recover"], expect=0)
    serve_and_post(NEW, "dfa8-after-bbf5", a, cfg_a, "by-dfa8-again", seen + [old_id])
    U.sh("dfa810fc status (roll forward)", [NEW, "operator", cfg_a, "status"], expect=0)
    ...
    U.sh("bbf52159 recover on the required store dfa810fc wrote", [OLD, "operator", cfg_b, "recover"], expect=0)
    old_b = serve_and_post(OLD, "bbf5-on-required", b, cfg_b, "by-bbf5-required", ["<qual-dfa8-required@example.invalid>"])
    U.sh("dfa810fc recover after bbf52159 wrote the required store", [NEW, "operator", cfg_b, "recover"], expect=0)
    U.sh("dfa810fc status (required, after bbf52159)", [NEW, "operator", cfg_b, "status"], expect=0)
