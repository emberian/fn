#!/usr/bin/env python3
"""qual-dfa810fc fact 1c, the three NEW rollback consequences, on COPIES only
(after upgrade_live.py made S/pristine, the dfa810fc install and the bbf52159 copy).
...
sys.path.insert(0, "/tank/fn/scratch/qual-dfa810fc")
import upgrade_live as U

S = U.S / "rollback3"
NEW = U.S / "fn-dfa810fc" / "bin" / "fn"
...
def part_accounts():
    ...
    rc, out = sh("dfa810fc account invite OFFLINE on a side copy", [NEW, "operator", cfgi, "account", "invite", "--expires", "3600"], quiet=True)
    ...
    rc, out = sh("dfa810fc account invite (live)", [NEW, "operator", cfg, "account", "invite", "--expires", "3600"], quiet=True)
    ...
    r2 = c.cmd("XREDEEM PASS qual-dfa810fc-friend", show=False)
    say("  XREDEEM over STARTTLS -> %s / PASS -> %s" % (r1[:40], r2[:40]))
    c.close()
    c = U.Nntp(port, cert, login=False)
    say("  AUTHINFO USER friend1 -> %s" % c.cmd("AUTHINFO USER friend1", show=False)[:40])
    say("  AUTHINFO PASS -> %s" % c.cmd("AUTHINFO PASS qual-dfa810fc-friend", show=False)[:40])
    ...
    c.close()
    U.stop(o, "dfa8-acct")
    rc, out = sh("dfa810fc account list", [NEW, "operator", cfg, "account", "list"])
    say("  the code is absent from account list: %s" % (code not in out))
    sh("dfa810fc status after the redeem", [NEW, "operator", cfg, "status"], expect=0)
...
def part_binding():
    ...
    rc, out = sh("dfa810fc principal bind qual P (live)", [NEW, "operator", cfg, "principal", "bind", U.LOGIN, U.P.hex()])
    ...
    sh("CONTROL: bbf52159 status after a dfa810fc owner ran with no binding (want 0)", [OLD, "operator", cfgc, "status"], expect=0)
    r = S / "bind-restored"
...
def part_prec1():
    ...
    for name in ("witness", "ordinary"):
        ...
        for verb in (["recover"], ["inspect", "<prec1-target@example.invalid>"], ["checkpoint"]):
            sh("dfa810fc store %s %s" % (name, " ".join(verb)), [NEWIMG, "--fn", "store", w, *verb],
               expect=(1 if name == "witness" else None))
        if name == "witness":
            sh("dfa810fc store witness repair-control (want refusal)", [NEWIMG, "--fn", "store", w, "repair-control"])
            cfg = S / "prec1.toml"
            cfg.write_text('[store]\npath = "%s"\n[listener]\nhost = "127.0.0.1"\nport = %d\n' % (w, U.free_port()))
            sh("dfa810fc operator health (offline) on the witness", [NEWIMG, "--fn", "operator", cfg, "health"])
            sh("dfa810fc operator run on the witness (want refusal, exit 1)", [NEWIMG, "--fn", "operator", cfg, "run"], expect=1)
