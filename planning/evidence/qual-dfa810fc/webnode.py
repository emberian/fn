#!/usr/bin/env python3
"""qual-dfa810fc web pass, node side (hbox): a scratch node on the PRODUCTION
...
S = Path("/tank/fn/scratch/qual-dfa810fc")
T = S / "tree"
...
W = S / "webnode"
EMBER_PW, GUEST_PW = "web-ember-dfa810fc", "web-guest-dfa810fc"
E = bytes([0xEE]) * 32
...
ROOT, MID = "<walk-root.dfa8@fn.example.invalid>", "<walk-mid.dfa8@fn.example.invalid>"
...
nntp_post("guest", GUEST_PW, ("Path: not-for-mail\r\nFrom: guest <guest@fn.example.invalid>\r\nNewsgroups: fn.agents\r\nSubject: Re: Daily walk\r\n"
          "Message-ID: <walk-tin.dfa8@fn.example.invalid>\r\nReferences: %s %s\r\nDate: Sat, 26 Sep 2026 06:31:00 +0000\r\n"
...
nntp_post("guest", GUEST_PW, (b"From: guest <guest@fn.example.invalid>\r\nNewsgroups: fn.agents\r\nSubject: walk notes\r\n"
          b"Message-ID: <walk-notes.dfa8@fn.example.invalid>\r\nDate: Sat, 26 Sep 2026 06:32:00 +0000\r\n\r\nnotes for the search\r\n"))
signed("cancel", "<walk-cancel.dfa8@fn.example.invalid>", "cmsg cancel " + MID, cancel=MID)
...
import fn_verify as FV
ring = {"format": "fn-verify-keyring-v1", "principals": [
    FV.keyring_entry(keys / "principal.bin", keys / "ed-public.bin", keys / "ml-public.pem", 1)]}
(W / "keyring.json").write_text(json.dumps(ring))
print("KEYRING E only (generation 1): guest's Q carrier is not in it", flush=True)
