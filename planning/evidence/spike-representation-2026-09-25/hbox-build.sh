#!/bin/sh
# usage: hbox-build.sh TREE   (developer image + census-hook image + prof image)
set -eu
ROOT=$1
ACL2=/tank/fn/toolchains/w28/acl2-literal-4g
CACHE=/tank/fn/certcache
export FN_OPENSSL_PREFIX=/tank/fn/toolchains/openssl-3.5.8 FN_SPIKE=1
...
# the census hook (rep-heap) spliced before the trust tag: inert unless FN_CENSUS_LOAD is set
awk '/^\(defttag nil\)/{while((getline l < "/tank/fn/scratch/rep-heap/census-hook.lisp")>0) print l} {print}' host/native/build.lisp > build/census-build.lisp
FN_ACL2=$ACL2 FN_NATIVE_PROFILE=developer FN_NATIVE_BUILD=build/census-build.lisp FN_NATIVE_IMAGE=build/fn-host-developer FN_NATIVE_LOG=build/freeze/native-build-developer.log swarm-build sh tools/build_native_host.sh
