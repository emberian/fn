#!/usr/bin/env python3
"""qual-69046a76 upgrade rehearsal on COPIES of the live node's store and fn.toml.
...
S = Path("/tank/fn/scratch/qual-69046a76/upgrade-live")
T = Path("/tank/fn/scratch/qual-69046a76/tree")
I = Path("/tank/fn/gates/qual-69046a76-20260926/build/images/69046a76798b8be6169eeed5a66cc46fbe399e51")
LIVE = Path("/tank/fn/node")
OPENSSL = "/tank/fn/scratch/qual-69046a76/bin/test-openssl"
LOGIN, SECRET = "qual", "qual-69046a76-rehearsal"
P = bytes([85]) * 32
...
# qual-69046a76: packaging/fn (every install's bin/fn) runs $FN_NATIVE_HOST when it is set,
...
def main():
    ...
    rc, out = sh("install-native 69046a76", ["env", "PREFIX=%s" % (S / "fn-69046a76"),
                                             "FN_NATIVE_HOST=%s" % (I / "fn-host"),
                                             "FN_NATIVE_SOURCE_REVISION=69046a76798b8be6169eeed5a66cc46fbe399e51",
    ...
    rc, new_status = sh("69046a76 status offline (per-article accounting)", [NEW, "operator", cfg, "status"], expect=0)
    ...
    for k in ("transactions", "articles", "transactions-used", "transactions-budget", "bytes-used", "history-bound",
        ...
        say("  1b %-22s bbf52159=%s 69046a76=%s" % (k, fo.get(k), fn_.get(k)))
    try:
    ...
    for line in new_status.splitlines():
        if line.startswith(("maintenance-reserve", "headroom", "profile", "open")):
            say("  1b line| " + line)
    sh("69046a76 health offline", [NEW, "operator", cfg, "health"])
    sh("69046a76 store needs-upgrade (format 8 unmarked)", [NEW, "operator", cfg, "store", "needs-upgrade"])
    sh("69046a76 store rollback-check KEPT format-7 config (in the live store)",
       [NEW, "operator", cfg, "store", "rollback-check", store / "config.json.format-7"])
    sh("69046a76 store rollback-check --snapshot of itself (nothing lost)",
    ...
    sh("bbf52159 status after 69046a76 opened with no commit", [OLD, "operator", cfg_o0, "status"])
    ...
    body1 = b"qual-69046a76 protected ordinary use: first post\n"
    ...
    sh("bbf52159 status on the store 69046a76 wrote (expected refusal)", [OLD, "operator", cfg_r0, "status"])
    sh("bbf52159 recover on the store 69046a76 wrote (expected refusal)", [OLD, "operator", cfg_r0, "recover"])
    sh("69046a76 store rollback-check KEPT format-7 config after the writes",
       [NEW, "operator", cfg, "store", "rollback-check", store / "config.json.format-7"])
    rc, out = sh("69046a76 store rollback-check --snapshot SNAPSHOT (the loss count)",
