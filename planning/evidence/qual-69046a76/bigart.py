#!/usr/bin/env python3
"""qual-69046a76: the served ARTICLE of a large article on a 4 MiB profile (bounds_join's red).
...
for spec in images:
    ...
    for n in sizes:
        ...
        r = subprocess.run([img, "--fn", "operator", str(cfg), "init", *os.environ.get("QUAL_INIT", "--max-article-octets 4194304").split(), "fn.test"], env=env, capture_output=True)
        ...
        while True:
            ...
            if not _l:
                sys.exit("owner exited before LISTENING: init=%r %r %r" % (r.returncode, r.stdout[-300:], r.stderr[-300:]))
