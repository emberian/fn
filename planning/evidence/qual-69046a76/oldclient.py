#!/usr/bin/env python3
"""qual-69046a76 item 5 (PKT-372): the b6759850 clients against a 69046a76 node.
...
gate's copy) and at 69046a76 over NNTP; `operator CONFIG post` through the
control socket with the b6759850 image and the 69046a76 image.  Prints what each
...
try:
    ...
    for label, tree in (("b6759850", OLDTREE), ("69046a76", NEWTREE)):
    ...
    run("69046a76 operator post (the original)", [NEWIMG, "--fn", "operator", cfg, "post", "--message-id", "<oldclient-op@example.invalid>", "--payload", art, "--group", "fn.test"])
    for label, img in (("b6759850", OLDIMG), ("69046a76", NEWIMG)):
    ...
    run("69046a76 hybrid-enroll P generation 1", [NEWIMG, "--fn", "hybrid-enroll", control, "1", k / "p.bin", k / "ed.pub", k / "ml.pub"])
    ...
    run("69046a76 hybrid-author (the original)", [NEWIMG, "--fn", "hybrid-author", control, "1", *orig])
    for label, img in (("b6759850", OLDIMG), ("69046a76", NEWIMG)):
