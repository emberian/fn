#!/usr/bin/env python3
"""qual-69046a76 rehearsal follow-up (after upgrade_live.py): the rollback to
bbf52159 WITHOUT a snapshot, which the rehearsal showed is possible on this
image (69046a76 writes the frontier in the format-2 bytes for every txid below
2^32, fn-bs-frontier-encode-impl), exercised end to end on copies:
  (a) bbf52159's owner on the store 69046a76 wrote: serve what 69046a76
      accepted, accept a new article; then 69046a76 reopens and serves it;
...
def main():
    ...
    # (a) plain deployed store (unmarked), written by 69046a76
    a = S / "rollback-binary"
    U.copytree(S / "w", a)
    cfg_a = U.config_for(a, U.free_port())
    U.sh("bbf52159 recover on the store 69046a76 wrote", [OLD, "operator", cfg_a, "recover"], expect=0)
    ...
    U.sh("69046a76 recover after bbf52159 wrote (roll forward again)", [NEW, "operator", cfg_a, "recover"], expect=0)
    serve_and_post(NEW, "dfa8-after-bbf5", a, cfg_a, "by-dfa8-again", seen + [old_id])
    U.sh("69046a76 status (roll forward)", [NEW, "operator", cfg_a, "status"], expect=0)
    ...
    U.sh("bbf52159 recover on the required store 69046a76 wrote", [OLD, "operator", cfg_b, "recover"], expect=0)
    old_b = serve_and_post(OLD, "bbf5-on-required", b, cfg_b, "by-bbf5-required", ["<qual-dfa8-required@example.invalid>"])
    U.sh("69046a76 recover after bbf52159 wrote the required store", [NEW, "operator", cfg_b, "recover"], expect=0)
    U.sh("69046a76 status (required, after bbf52159)", [NEW, "operator", cfg_b, "status"], expect=0)
