#!/bin/sh
# qual-69046a76 standing cases, part 3, scale-1m with A=4 MiB and R=32 MiB (the DTN images omit operator run: prod and dev only)
set -u
. /tank/fn/scratch/qual-69046a76/env.sh
R=$S/standing
cd $S/tree
SCALE="--profile scale --max-transactions 1048576 --max-history-octets 4294967296 --max-article-octets 4194304 --max-record-octets 33554432"
