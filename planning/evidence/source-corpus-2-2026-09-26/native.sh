...
export FN_NATIVE_HOST=$S/tree/build/fn-host-developer FN_NATIVE_DTN_HOST=$S/tree/build/fn-host-dtn-developer FN_DTN7_REPO=/tank/fn/dtn7/repo
rm -rf $S/diag$1; mkdir -p $S/diag$1
export FN_NATIVE_TEST_DIAGNOSTIC_DIR=$S/diag$1
T0=$(date +%s); python3 -m unittest -v tests.test_native_source_corpus_bp > $S/native-$1.log 2>&1; echo "rc=$? wall=$(($(date +%s)-T0))" >> $S/native-$1.log
