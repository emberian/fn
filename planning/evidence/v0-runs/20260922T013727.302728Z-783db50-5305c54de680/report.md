...
| three outcomes a | accepted=4 refused=4 uncertain=not-built |
| three outcomes b | accepted=4 refused=4 uncertain=not-built |
| transit | IHAVE -> 'no answer'; CAPABILITIES lists IHAVE: None |
| rows | 192 rows: 83 accepted, 12 refused, 4 uncertain, 56 not exercised, 37 not built, 4 disagreed |
...
192 rows: 83 accepted, 12 refused, 4 uncertain, 56 not exercised, 37 not built; 4 row(s) did not do what they were designed to do. Every count here is `tools/v0_matrix.py`'s over the rows below, and `planning/evidence/v0-runs/20260922T013727.302728Z-783db50-5305c54de680/matrix.json` carries the same rows with their digest.


**Who saw it.** 0 of the 99 outcome rows were observed by something that is not fn's own code; 99 were observed by fn talking to fn. A feature that only fn's own client has ever seen is a weaker claim than "usable between two peered servers" reads, and every row carries the client that saw it in its `client` field. Clients in this run: fn CLI (exit code) (19); the matrix's raw-socket driver (80).
...
| the three outcomes on the operator surface (F-OUT) | 2 | 0 | 4 | 0 | 2 | 4 |
...
| transit inbound, A to B and B to A (F-TRANSIT) | 0 | 0 | 0 | 2 | 24 | 0 |
...
| `V0-NODE-STOP-A` | the service stops and releases the store | **accepted** | accepted | yes | fn only | stop rc=0; native status afterwards rc=0 transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment |
| `V0-NODE-STOP-B` | the service stops and releases the store | **accepted** | accepted | yes | fn only | stop rc=0; native status afterwards rc=0 transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment |
| `V0-NODE-LOOPBACK` | a non-loopback listener host is refused rather than silently bound | **not-exercised** | refused | - | - | (not run) |
| `V0-OUT-ACCEPTED-A` | an accepted post exits 0 | **uncertain** | accepted | NO | fn only | rc=4 fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article' |
| `V0-OUT-ACCEPTED-B` | an accepted post exits 0 | **uncertain** | accepted | NO | fn only | rc=4 fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article' |
| `V0-OUT-REFUSED-A` | a lookup of an article the node does not hold exits 1 | **uncertain** | refused | NO | fn only | rc=4 fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article' |
| `V0-OUT-REFUSED-B` | a lookup of an article the node does not hold exits 1 | **uncertain** | refused | NO | fn only | rc=4 fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article' |
...
| `V0-OUT-RECOVER-A` | recover after the uncertain publication exits 0 | **accepted** | accepted | yes | fn only | rc=0 recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none |
| `V0-OUT-RECOVER-B` | recover after the uncertain publication exits 0 | **accepted** | accepted | yes | fn only | rc=0 recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none |
...
| `V0-POST-READBACK-B` | the posting connection's next GROUP already counts the article | **accepted** | accepted | yes | fn only | GROUP before=211 1 1 1 fn.letters after=211 2 1 2 fn.letters |
| `V0-POST-FRESH-A` | a fresh connection reads it back by Message-ID | **accepted** | accepted | yes | fn only | 220 0 <native-matrix-c32cb94a7b82-a@example.invalid> article follows |
| `V0-POST-FRESH-B` | a fresh connection reads it back by Message-ID | **accepted** | accepted | yes | fn only | 220 0 <native-matrix-4557f388c3ec-b@example.invalid> article follows |
...
| `V0-POST-CLOCK-A` | a duplicate POST is refused as an article and does not cost the node its clock | **accepted** | accepted | yes | fn only | the duplicate answered '441 posting failed; the article was refused' and DATE afterwards answered '111 20260922013751' |
| `V0-POST-CLOCK-B` | a duplicate POST is refused as an article and does not cost the node its clock | **accepted** | accepted | yes | fn only | the duplicate answered '441 posting failed; the article was refused' and DATE afterwards answered '111 20260922013753' |
| `V0-POST-CONCURRENT` | a second reader stays live across another connection's whole POST | **accepted** | accepted | yes | fn only | watcher mid-post=211 2 1 2 fn.letters commit=240 article received OK |
...
| `V0-READ-GROUP-B` | GROUP | **accepted** | accepted | yes | fn only | 211 2 1 2 fn.letters |
| `V0-READ-LISTGROUP-A` | LISTGROUP with a range | **accepted** | accepted | yes | fn only | 211 1 1 1 fn.letters list follows |
| `V0-READ-LISTGROUP-B` | LISTGROUP with a range | **accepted** | accepted | yes | fn only | 211 2 1 2 fn.letters list follows |
| `V0-READ-ARTICLE-A` | ARTICLE by number | **accepted** | accepted | yes | fn only | 220 1 <native-matrix-c32cb94a7b82-a@example.invalid> article follows |
| `V0-READ-ARTICLE-B` | ARTICLE by number | **accepted** | accepted | yes | fn only | 220 1 <native-matrix-c32cb94a7b82-a@example.invalid> article follows |
| `V0-READ-HEAD-A` | HEAD by number | **accepted** | accepted | yes | fn only | 221 1 <native-matrix-c32cb94a7b82-a@example.invalid> headers follow |
| `V0-READ-HEAD-B` | HEAD by number | **accepted** | accepted | yes | fn only | 221 1 <native-matrix-c32cb94a7b82-a@example.invalid> headers follow |
| `V0-READ-BODY-A` | BODY by number | **accepted** | accepted | yes | fn only | 222 1 <native-matrix-c32cb94a7b82-a@example.invalid> body follows |
| `V0-READ-BODY-B` | BODY by number | **accepted** | accepted | yes | fn only | 222 1 <native-matrix-c32cb94a7b82-a@example.invalid> body follows |
| `V0-READ-STAT-A` | STAT by number | **accepted** | accepted | yes | fn only | 223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved |
| `V0-READ-STAT-B` | STAT by number | **accepted** | accepted | yes | fn only | 223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved |
| `V0-READ-ARTICLE-MSGID-A` | ARTICLE by Message-ID | **accepted** | accepted | yes | fn only | 220 0 <native-matrix-c32cb94a7b82-a@example.invalid> article follows |
| `V0-READ-ARTICLE-MSGID-B` | ARTICLE by Message-ID | **accepted** | accepted | yes | fn only | 220 0 <native-matrix-4557f388c3ec-b@example.invalid> article follows |
...
| `V0-READ-NEXT-B` | NEXT moves the cursor | **accepted** | - | - | fn only | 223 2 <native-matrix-4557f388c3ec-b@example.invalid> retrieved |
| `V0-READ-LAST-A` | LAST moves the cursor back | **refused** | - | - | fn only | 422 no previous article |
| `V0-READ-LAST-B` | LAST moves the cursor back | **accepted** | - | - | fn only | 223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved |
| `V0-READ-DATE-A` | DATE | **accepted** | accepted | yes | fn only | 111 20260922013751 |
| `V0-READ-DATE-B` | DATE | **accepted** | accepted | yes | fn only | 111 20260922013753 |
...
| `V0-READ-FRAMING-A` | a command split across two TCP segments is answered once, the same way | **accepted** | accepted | yes | fn only | DATE split across two segments answered '111 20260922013751' against '111 20260922013751' |
| `V0-READ-FRAMING-B` | a command split across two TCP segments is answered once, the same way | **accepted** | accepted | yes | fn only | DATE split across two segments answered '111 20260922013753' against '111 20260922013753' |
...
- `V0-CAP-SET-A` (not-exercised): the image's store entry did not initialise a scratch store (rc=4 store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/a/capacity/store')
- `V0-CAP-SET-B` (not-exercised): the image's store entry did not initialise a scratch store (rc=4 store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/b/capacity/store')
- `V0-CAP-REFUSE-A` (not-exercised): the image's store entry did not initialise a scratch store (rc=4 store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/a/capacity/store')
- `V0-CAP-REFUSE-B` (not-exercised): the image's store entry did not initialise a scratch store (rc=4 store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/b/capacity/store')
...
- `V0-READ-NEXT-B`: the expected outcome depends on how many articles the group holds; the row records the count it was run against; served by `native-operator` on port 11191; the group held 2 article(s) when this ran, and with one article 421/422 is the correct answer
...
- `V0-READ-LAST-B`: the expected outcome depends on where the cursor was; served by `native-operator` on port 11191; the group held 2 article(s) when this ran, and with one article 421/422 is the correct answer
...
- `V0-CAP-SET-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; mkdir -p $HOME/fn-deploy/native915-783db50/a/capacity && env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native store '$HOME/fn-deploy/native915-783db50/a/capacity/store' init fn.letters && printf '[store]\npath = "%s"\n' '$HOME/fn-deploy/native915-783db50/a/capaci...`
- `V0-CAP-SET-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; mkdir -p $HOME/fn-deploy/native915-783db50/b/capacity && env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native store '$HOME/fn-deploy/native915-783db50/b/capacity/store' init fn.letters && printf '[store]\npath = "%s"\n' '$HOME/fn-deploy/native915-783db50/b/capaci...`
- `V0-CAP-REFUSE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; mkdir -p $HOME/fn-deploy/native915-783db50/a/capacity && env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native store '$HOME/fn-deploy/native915-783db50/a/capacity/store' init fn.letters && printf '[store]\npath = "%s"\n' '$HOME/fn-deploy/native915-783db50/a/capaci...`
- `V0-CAP-REFUSE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; mkdir -p $HOME/fn-deploy/native915-783db50/b/capacity && env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native store '$HOME/fn-deploy/native915-783db50/b/capacity/store' init fn.letters && printf '[store]\npath = "%s"\n' '$HOME/fn-deploy/native915-783db50/b/capaci...`
...
- `V0-POST-OPEN-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-OPEN-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
- `V0-POST-COMMIT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-COMMIT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
- `V0-POST-READBACK-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-READBACK-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
- `V0-POST-FRESH-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-FRESH-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
- `V0-POST-DUPLICATE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-DUPLICATE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
- `V0-POST-CLOCK-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
- `V0-POST-CLOCK-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
...
- `V0-READ-CAPABILITIES-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-CAPABILITIES-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-MODE-READER-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-MODE-READER-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-ACTIVE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-ACTIVE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-NEWSGROUPS-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-NEWSGROUPS-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-OVERVIEW-FMT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-OVERVIEW-FMT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-ACTIVE-TIMES-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-ACTIVE-TIMES-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-HEADERS-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LIST-HEADERS-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-GROUP-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-GROUP-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LISTGROUP-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LISTGROUP-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HEAD-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HEAD-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-BODY-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-BODY-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-STAT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-STAT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-MSGID-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-MSGID-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-ABSENT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-ARTICLE-ABSENT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-OVER-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-OVER-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-OVER-RANGE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-OVER-RANGE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HDR-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HDR-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XOVER-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XOVER-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XHDR-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XHDR-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XPAT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-XPAT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-NEXT-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-NEXT-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LAST-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-LAST-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-DATE-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-DATE-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HELP-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-HELP-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-UNKNOWN-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-UNKNOWN-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-FRAMING-A`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
- `V0-READ-FRAMING-B`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
...
- `V0-TRANSIT-CHECK-FRESH-AB`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11190 --to-port 11191 --msgid '<alpha@a.example.invalid>' --group fn.letters --loop-msgid '<loop-ab@example.invalid>' --loop-identity b.gate.example.invalid`
- `V0-TRANSIT-CHECK-FRESH-BA`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11191 --to-port 11190 --msgid '<beta@b.example.invalid>' --group fn.letters --loop-msgid '<loop-ba@example.invalid>' --loop-identity a.gate.example.invalid`
- `V0-TRANSIT-TAKETHIS-AB`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11190 --to-port 11191 --msgid '<alpha@a.example.invalid>' --group fn.letters --loop-msgid '<loop-ab@example.invalid>' --loop-identity b.gate.example.invalid`
- `V0-TRANSIT-TAKETHIS-BA`: `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11191 --to-port 11190 --msgid '<beta@b.example.invalid>' --group fn.letters --loop-msgid '<loop-ba@example.invalid>' --loop-identity a.gate.example.invalid`
...
| 2 | acquire deploy lock | 0 | `` | 0.5 |
| 3 | ship archive | 0 | `` | 7.9 |
| 4 | make run dir | 0 | `` | 0.5 |
...
| 14 | node B native operator status | 0 | `transactions=0 articles=0 staging-orphans=0 unsigned-legacy-experiment` | 0.7 |
| 15 | node A group create fn.matrix | 0 | `configured generation=2 record=00000002.cfg verification=VERIFIED` | 0.5 |
| 16 | node A group create fn.matrix.throwaway | 0 | `configured generation=3 record=00000003.cfg verification=VERIFIED` | 0.6 |
| 17 | node A group retire fn.matrix.throwaway | 0 | `configured generation=4 record=00000004.cfg verification=VERIFIED` | 0.5 |
| 18 | node A group retire an unserved group | 1 | `refused operator group administrative configuration refused: NO-SUCH-GROUP` | 0.7 |
| 19 | node A capacity scratch store | 4 | `store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/a/capacity/store'` | 0.7 |
| 20 | node B group create fn.matrix | 0 | `configured generation=2 record=00000002.cfg verification=VERIFIED` | 0.5 |
| 21 | node B group create fn.matrix.throwaway | 0 | `configured generation=3 record=00000003.cfg verification=VERIFIED` | 0.7 |
| 22 | node B group retire fn.matrix.throwaway | 0 | `configured generation=4 record=00000004.cfg verification=VERIFIED` | 0.5 |
| 23 | node B group retire an unserved group | 1 | `refused operator group administrative configuration refused: NO-SUCH-GROUP` | 0.6 |
| 24 | node B capacity scratch store | 4 | `store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/b/capacity/store'` | 0.7 |
| 25 | node B configured port | 0 | `11191` | 0.6 |
| 26 | node A peer record for B | 0 | `configured generation=5 record=00000005.cfg verification=VERIFIED` | 0.7 |
| 27 | node A configured port | 0 | `11190` | 0.6 |
| 28 | node B peer record for A | 0 | `configured generation=5 record=00000005.cfg verification=VERIFIED` | 0.5 |
| 29 | start server (node A (native-operator), a-main) | 0 | `LISTENING 11190` | 1.4 |
| 30 | node A pid | 0 | `2269302` | 1.1 |
| 31 | start server (node B (native-operator), b-main) | 0 | `LISTENING 11191` | 1.3 |
| 32 | node B pid | 0 | `2269705` | 1.1 |
| 33 | node A server is alive | 0 | `ALIVE` | 0.2 |
| 34 | node A serves fn.matrix | 0 | `{"groups": {"fn.matrix": "211 0 1 0 fn.matrix"}, "present": {}, "absent": {}, "ok": true}` | 0.3 |
| 35 | node A server is alive | 0 | `ALIVE` | 0.3 |
| 36 | node A POST cycle | 0 | `{"GROUP BEFORE": "211 0 1 0 fn.letters", "POST": "340 send article to be posted", "COMMIT": "240 article received OK", "GROUP AFTER": "211 1 1 1 fn.letters", "FRESH ARTICLE": "220 0 <native-matrix-c32` | 0.4 |
| 37 | node A server is alive | 0 | `ALIVE` | 0.4 |
| 38 | node A reader surface | 0 | `{"greeting": "200 fn-nntp experimental server ready", "CAPABILITIES": "101 capability list follows", "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES ` | 0.7 |
| 39 | node A server is alive | 0 | `ALIVE` | 1.1 |
| 40 | install alpha.article | 0 | `` | 0.5 |
| 41 | node A outcome accepted | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article'` | 1.3 |
| 42 | install alpha.article | 0 | `` | 0.9 |
| 43 | node A outcome refused | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article'` | 0.8 |
| 44 | install stream-a.article | 0 | `` | 0.3 |
| 45 | node A seed <stream-a@example.invalid> | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/stream-a.article'` | 0.3 |
| 46 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 47 | node B serves fn.matrix | 0 | `{"groups": {"fn.matrix": "211 0 1 0 fn.matrix"}, "present": {}, "absent": {}, "ok": true}` | 0.3 |
| 48 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 49 | node B POST cycle | 0 | `{"GROUP BEFORE": "211 1 1 1 fn.letters", "POST": "340 send article to be posted", "COMMIT": "240 article received OK", "GROUP AFTER": "211 2 1 2 fn.letters", "FRESH ARTICLE": "220 0 <native-matrix-455` | 0.4 |
| 50 | node B server is alive | 0 | `ALIVE` | 0.2 |
| 51 | node B reader surface | 0 | `{"greeting": "200 fn-nntp experimental server ready", "CAPABILITIES": "101 capability list follows", "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES ` | 0.8 |
| 52 | node B server is alive | 0 | `ALIVE` | 0.2 |
| 53 | install beta.article | 0 | `` | 0.3 |
| 54 | node B outcome accepted | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article'` | 0.3 |
| 55 | install beta.article | 0 | `` | 0.3 |
| 56 | node B outcome refused | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article'` | 0.3 |
| 57 | install stream-b.article | 0 | `` | 0.4 |
| 58 | node B seed <stream-b@example.invalid> | 4 | `fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/stream-b.article'` | 0.4 |
| 59 | node A server is alive | 0 | `ALIVE` | 0.2 |
| 60 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 61 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 62 | transit AB: offer <alpha@a.example.invalid> from A to B | 1 | `{"msgid": "<alpha@a.example.invalid>", "source": "430 no article with that message-id", "ok": false, "transit": null, "reason": "the source node does not serve <alpha@a.example.invalid>"}` | 0.4 |
| 63 | node A server is alive | 0 | `ALIVE` | 0.3 |
| 64 | transit BA: offer <beta@b.example.invalid> from B to A | 1 | `{"msgid": "<beta@b.example.invalid>", "source": "430 no article with that message-id", "ok": false, "transit": null, "reason": "the source node does not serve <beta@b.example.invalid>"}` | 0.3 |
| 65 | node A server is alive | 0 | `ALIVE` | 0.5 |
| 66 | node A capability pins | 0 | `{"advertised": ["VERSION", "READER", "POST", "OVER", "HDR", "LIST", "IMPLEMENTATION", "IHAVE", "STREAMING"], "answered": {"READER": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "PO` | 0.3 |
| 67 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 68 | node B capability pins | 0 | `{"advertised": ["VERSION", "READER", "POST", "OVER", "HDR", "LIST", "IMPLEMENTATION", "IHAVE", "STREAMING"], "answered": {"READER": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "PO` | 0.4 |
| 69 | node A server is alive | 0 | `ALIVE` | 0.3 |
| 70 | node B server is alive | 0 | `ALIVE` | 0.2 |
| 71 | a second reader across node A's POST | 0 | `{"WATCHER BEFORE": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "WATCHER MID": "211 2 1 2 fn.letters", "WATCHER ARTICLE MID": "223 1 <native-matrix-c32cb94a7b82-a@example.invalid> ` | 0.4 |
| 72 | node A server is alive | 0 | `ALIVE` | 0.2 |
| 73 | node B server is alive | 0 | `ALIVE` | 0.3 |
| 74 | offline group create while the owner holds the store | 1 | `refused operator group store is already locked` | 0.4 |
| 75 | native public peering/restart witness | 0 | `NATIVE-PEERING-EXPECTED-RUNTIME b115fe956aadee603459fac401e1ff2cc39321e14848544a0fe438788a2fc6d5` | 6.3 |
| 76 | stop server node A (main) | 0 | `stopped` | 2.1 |
| 77 | node A store after the stop | 0 | `transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment` | 0.3 |
| 78 | node A recover after the stop | 0 | `recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none` | 0.5 |
| 79 | node A log tail | 0 | `CONTROL /home/ember/fn-native915-matrix/a/control.sock` | 0.3 |
| 80 | stop server node B (main) | 0 | `stopped` | 1.2 |
| 81 | node B store after the stop | 0 | `transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment` | 0.9 |
| 82 | node B recover after the stop | 0 | `recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none` | 0.5 |
| 83 | node B log tail | 0 | `CONTROL /home/ember/fn-native915-matrix/b/control.sock` | 0.3 |
| 84 | peer remove a peer that is not there | 1 | `refused operator peer administrative configuration refused: NO-SUCH-PEER` | 0.4 |
| 85 | peer remove the configured peer | 0 | `configured generation=6 record=00000006.cfg verification=VERIFIED` | 0.4 |
| 86 | stop server node A (main) | 0 | `stopped` | 0.2 |
| 87 | stop the tap in front of node A | 0 | `stopped` | 0.3 |
| 88 | stop server node B (main) | 0 | `stopped` | 0.2 |
| 89 | stop the tap in front of node B | 0 | `stopped` | 0.3 |
| 90 | stray fn processes | 0 | `CLEAN` | 0.4 |
| 91 | remove the deploy tree | 0 | `` | 0.3 |
| 92 | release deploy lock | 0 | `` | 0.3 |
...
20. **node B group create fn.matrix** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml group create fn.matrix`
21. **node B group create fn.matrix.throwaway** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml group create fn.matrix.throwaway`
22. **node B group retire fn.matrix.throwaway** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml group retire fn.matrix.throwaway`
23. **node B group retire an unserved group** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml group retire fn.not.served`
24. **node B capacity scratch store** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; mkdir -p $HOME/fn-deploy/native915-783db50/b/capacity && env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native store '$HOME/fn-deploy/native915-783db50/b/capacity/store' init fn.letters && printf '[store]\npath = "%s"\n' '$HOME/fn-deploy/native915-783db50/b/capaci...`
25. **node B configured port** -- `sed -n 's/^port *= *\([0-9][0-9]*\).*/\1/p' /home/ember/fn-native915-matrix/b/fn.toml | head -1`
26. **node A peer record for B** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml peer add b b.gate.example.invalid 127.0.0.1 11191 'fn.*' 'fn.*' 127.0.0.1 true`
27. **node A configured port** -- `sed -n 's/^port *= *\([0-9][0-9]*\).*/\1/p' /home/ember/fn-native915-matrix/a/fn.toml | head -1`
28. **node B peer record for A** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml peer add a a.gate.example.invalid 127.0.0.1 11190 'fn.*' 'fn.*' 127.0.0.1 true`
29. **start server (node A (native-operator), a-main)** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; rm -f $HOME/fn-deploy/native915-783db50/a/server-a-main.log ; nohup env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml run > $HOME/fn-deploy/native915-783db50/a/server-a-main.log 2>&1 < /dev/null & ; echo $! > ...`
30. **node A pid** -- `cat $HOME/fn-deploy/native915-783db50/a/server.pid`
31. **start server (node B (native-operator), b-main)** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; rm -f $HOME/fn-deploy/native915-783db50/b/server-b-main.log ; nohup env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml run > $HOME/fn-deploy/native915-783db50/b/server-b-main.log 2>&1 < /dev/null & ; echo $! > ...`
32. **node B pid** -- `cat $HOME/fn-deploy/native915-783db50/b/server.pid`
33. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
34. **node A serves fn.matrix** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py presence --port 11190 --groups fn.matrix`
35. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
36. **node A POST cycle** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --user '' --secret ''`
37. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
38. **node A reader surface** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11190 --group fn.letters --msgid '<native-matrix-c32cb94a7b82-a@example.invalid>' --absent '<absent@example.invalid>'`
39. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
40. **install alpha.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/a/alpha.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IG91dGNvbWUsIHdyaXR0ZW4gb24g ; QQ0KTmV3c2dyb3VwczogZm4ubGV0dGVycw0KRGF0ZTogVHVlLCAyMiBTZXAgMjAyNiAwMTozNzo1 ; OCArMDAwMA0KTWVzc2FnZS1JRDogPGFscGhhQGEuZXhhbXBsZS5pbnZhbGlkPg0KDQpXcml0dGVu ; IG9uIG5vZGUgQSBieSB0aGUgdjAgbWF0cml4IHRocm91Z2ggdGhlIG5hdGl2ZSBvcGVyYXRvci4N ; C...`
41. **node A outcome accepted** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml post --message-id '<alpha@a.example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/a/alpha.article' --group fn.letters`
42. **install alpha.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/a/alpha.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IG91dGNvbWUsIHdyaXR0ZW4gb24g ; QQ0KTmV3c2dyb3VwczogZm4ubGV0dGVycw0KRGF0ZTogVHVlLCAyMiBTZXAgMjAyNiAwMTozODow ; MCArMDAwMA0KTWVzc2FnZS1JRDogPGFscGhhQGEuZXhhbXBsZS5pbnZhbGlkPg0KDQpXcml0dGVu ; IG9uIG5vZGUgQSBieSB0aGUgdjAgbWF0cml4IHRocm91Z2ggdGhlIG5hdGl2ZSBvcGVyYXRvci4N ; C...`
43. **node A outcome refused** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml post --message-id '<alpha@a.example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/a/alpha.article' --group fn.letters`
44. **install stream-a.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/a/stream-a.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IGZvciB0aGUgc3RyZWFtaW5nIG9m ; ZmVyDQpOZXdzZ3JvdXBzOiBmbi5sZXR0ZXJzDQpEYXRlOiBUdWUsIDIyIFNlcCAyMDI2IDAxOjM4 ; OjAyICswMDAwDQpNZXNzYWdlLUlEOiA8c3RyZWFtLWFAZXhhbXBsZS5pbnZhbGlkPg0KDQpTZWVk ; ZWQgb24gbm9kZSBBIGJ5IHRoZSB2MCBtYXRyaXguDQo= ; FN_GATE_EOF ; chmod 644 $HOME/...`
45. **node A seed <stream-a@example.invalid>** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml post --message-id '<stream-a@example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/a/stream-a.article' --group fn.letters`
46. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
47. **node B serves fn.matrix** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py presence --port 11191 --groups fn.matrix`
48. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
49. **node B POST cycle** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py postcycle --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --user '' --secret ''`
50. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
51. **node B reader surface** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py surface --port 11191 --group fn.letters --msgid '<native-matrix-4557f388c3ec-b@example.invalid>' --absent '<absent@example.invalid>'`
52. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
53. **install beta.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/b/beta.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IG91dGNvbWUsIHdyaXR0ZW4gb24g ; Qg0KTmV3c2dyb3VwczogZm4ubGV0dGVycw0KRGF0ZTogVHVlLCAyMiBTZXAgMjAyNiAwMTozODow ; NSArMDAwMA0KTWVzc2FnZS1JRDogPGJldGFAYi5leGFtcGxlLmludmFsaWQ+DQoNCldyaXR0ZW4g ; b24gbm9kZSBCIGJ5IHRoZSB2MCBtYXRyaXggdGhyb3VnaCB0aGUgbmF0aXZlIG9wZXJhdG9yLg0K ; FN...`
54. **node B outcome accepted** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml post --message-id '<beta@b.example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/b/beta.article' --group fn.letters`
55. **install beta.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/b/beta.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IG91dGNvbWUsIHdyaXR0ZW4gb24g ; Qg0KTmV3c2dyb3VwczogZm4ubGV0dGVycw0KRGF0ZTogVHVlLCAyMiBTZXAgMjAyNiAwMTozODow ; NiArMDAwMA0KTWVzc2FnZS1JRDogPGJldGFAYi5leGFtcGxlLmludmFsaWQ+DQoNCldyaXR0ZW4g ; b24gbm9kZSBCIGJ5IHRoZSB2MCBtYXRyaXggdGhyb3VnaCB0aGUgbmF0aXZlIG9wZXJhdG9yLg0K ; FN...`
56. **node B outcome refused** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml post --message-id '<beta@b.example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/b/beta.article' --group fn.letters`
57. **install stream-b.article** -- `base64 -d > $HOME/fn-deploy/native915-783db50/b/stream-b.article <<'FN_GATE_EOF' ; RnJvbTogZ2F0ZUBleGFtcGxlLmludmFsaWQNClN1YmplY3Q6IGZvciB0aGUgc3RyZWFtaW5nIG9m ; ZmVyDQpOZXdzZ3JvdXBzOiBmbi5sZXR0ZXJzDQpEYXRlOiBUdWUsIDIyIFNlcCAyMDI2IDAxOjM4 ; OjA2ICswMDAwDQpNZXNzYWdlLUlEOiA8c3RyZWFtLWJAZXhhbXBsZS5pbnZhbGlkPg0KDQpTZWVk ; ZWQgb24gbm9kZSBCIGJ5IHRoZSB2MCBtYXRyaXguDQo= ; FN_GATE_EOF ; chmod 644 $HOME/...`
58. **node B seed <stream-b@example.invalid>** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml post --message-id '<stream-b@example.invalid>' --payload '$HOME/fn-deploy/native915-783db50/b/stream-b.article' --group fn.letters`
59. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
60. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
61. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
62. **transit AB: offer <alpha@a.example.invalid> from A to B** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11190 --to-port 11191 --msgid '<alpha@a.example.invalid>' --group fn.letters --loop-msgid '<loop-ab@example.invalid>' --loop-identity b.gate.example.invalid`
63. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
64. **transit BA: offer <beta@b.example.invalid> from B to A** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/feed.py relay --from-port 11191 --to-port 11190 --msgid '<beta@b.example.invalid>' --group fn.letters --loop-msgid '<loop-ba@example.invalid>' --loop-identity a.gate.example.invalid`
65. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
66. **node A capability pins** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py pins --port 11190 --group fn.letters --user '' --secret ''`
67. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
68. **node B capability pins** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py pins --port 11191 --group fn.letters --user '' --secret ''`
69. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
70. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
71. **a second reader across node A's POST** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; python3 $HOME/fn-deploy/native915-783db50/gate-run/matrix.py concurrent --port 11190 --group fn.letters --msgid '<concurrent@example.invalid>' --user '' --secret ''`
72. **node A server is alive** -- `kill -0 2269302 2>/dev/null && echo ALIVE || echo DEAD`
73. **node B server is alive** -- `kill -0 2269705 2>/dev/null && echo ALIVE || echo DEAD`
74. **offline group create while the owner holds the store** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml group create fn.matrix.offline`
75. **native public peering/restart witness** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; image=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host ; digest() { ; if command -v sha256sum >/dev/null 2>&1; then sha256sum "$1" | awk '{print $1}' ; else shasum -a 256 "$1" | awk '{print $1}'; fi ; } ; launcher_before=$(digest packaging/fn-native) ; runtime_before=$(digest "$image") ; core_befo...`
76. **stop server node A (main)** -- `if [ -f $HOME/fn-deploy/native915-783db50/a/server.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/a/server.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 20); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/a/server.pid ; fi ; echo stopped`
77. **node A store after the stop** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml status`
78. **node A recover after the stop** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml recover`
79. **node A log tail** -- `tail -12 $HOME/fn-deploy/native915-783db50/a/server-a-main.log 2>/dev/null || echo NO-LOG`
80. **stop server node B (main)** -- `if [ -f $HOME/fn-deploy/native915-783db50/b/server.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/b/server.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 20); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/b/server.pid ; fi ; echo stopped`
81. **node B store after the stop** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml status`
82. **node B recover after the stop** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/b/fn.toml recover`
83. **node B log tail** -- `tail -12 $HOME/fn-deploy/native915-783db50/b/server-b-main.log 2>/dev/null || echo NO-LOG`
84. **peer remove a peer that is not there** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml peer remove no-such-peer`
85. **peer remove the configured peer** -- `cd $HOME/fn-deploy/native915-783db50 || exit 9 ; env FN_NATIVE_HOST=/home/ember/fn-gates/freeze-f7190d69/build/images/915d5c729877eddee7dd3f72eadad21cca463d1a/fn-host packaging/fn-native operator /home/ember/fn-native915-matrix/a/fn.toml peer remove b`
86. **stop server node A (main)** -- `if [ -f $HOME/fn-deploy/native915-783db50/a/server.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/a/server.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 20); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/a/server.pid ; fi ; echo stopped`
87. **stop the tap in front of node A** -- `if [ -f $HOME/fn-deploy/native915-783db50/a/tap.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/a/tap.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 10); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/a/tap.pid ; fi ; echo stopped`
88. **stop server node B (main)** -- `if [ -f $HOME/fn-deploy/native915-783db50/b/server.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/b/server.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 20); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/b/server.pid ; fi ; echo stopped`
89. **stop the tap in front of node B** -- `if [ -f $HOME/fn-deploy/native915-783db50/b/tap.pid ]; then ; pid=$(cat $HOME/fn-deploy/native915-783db50/b/tap.pid) ; kill $pid 2>/dev/null || true ; for i in $(seq 1 10); do kill -0 $pid 2>/dev/null || break; sleep 1; done ; kill -9 $pid 2>/dev/null || true ; rm -f $HOME/fn-deploy/native915-783db50/b/tap.pid ; fi ; echo stopped`
90. **stray fn processes** -- `pgrep -f 'fn-deploy/native915-783db50' >/dev/null 2>&1 && echo STRAY || echo CLEAN`
91. **remove the deploy tree** -- `rm -rf $HOME/fn-deploy/native915-783db50`
92. **release deploy lock** -- `rmdir $HOME/fn-deploy/.locks/native915-783db50.lock`
...
Verdict of this run: **held** (2 limitation).

| assertion | verdict | what it says |
| --- | --- | --- |
| node-a-seeding-stream-a-example-invalid-exited-4-fault-operator-post-errno-2-no- | limitation | node A: seeding <stream-a@example.invalid> exited 4 (fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/stream-a.article'); the streaming rows that offer it are recorded against that |
| node-b-seeding-stream-b-example-invalid-exited-4-fault-operator-post-errno-2-no- | limitation | node B: seeding <stream-b@example.invalid> exited 4 (fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/stream-b.article'); the streaming rows that offer it are recorded against that |

## What was NOT exercised

- node A: seeding <stream-a@example.invalid> exited 4 (fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/stream-a.article'); the streaming rows that offer it are recorded against that
- node B: seeding <stream-b@example.invalid> exited 4 (fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/stream-b.article'); the streaming rows that offer it are recorded against that
...
--- 19 node A capacity scratch store (rc=4)
store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/a/capacity/store'
--- 20 node B group create fn.matrix (rc=0)
configured generation=2 record=00000002.cfg verification=VERIFIED
accepted operator group
--- 21 node B group create fn.matrix.throwaway (rc=0)
configured generation=3 record=00000003.cfg verification=VERIFIED
accepted operator group
--- 22 node B group retire fn.matrix.throwaway (rc=0)
configured generation=4 record=00000004.cfg verification=VERIFIED
accepted operator group
--- 23 node B group retire an unserved group (rc=1)
refused operator group administrative configuration refused: NO-SUCH-GROUP
--- 24 node B capacity scratch store (rc=4)
store: [Errno 2] No such file or directory: '/home/ember/fn-deploy/native915-783db50/$HOME/fn-deploy/native915-783db50/b/capacity/store'
--- 25 node B configured port (rc=0)
11191
--- 26 node A peer record for B (rc=0)
configured generation=5 record=00000005.cfg verification=VERIFIED
accepted operator peer
--- 27 node A configured port (rc=0)
11190
--- 28 node B peer record for A (rc=0)
configured generation=5 record=00000005.cfg verification=VERIFIED
accepted operator peer
--- 29 start server (node A (native-operator), a-main) (rc=0)
LISTENING 11190
--- 30 node A pid (rc=0)
2269302
--- 31 start server (node B (native-operator), b-main) (rc=0)
LISTENING 11191
--- 32 node B pid (rc=0)
2269705
--- 33 node A server is alive (rc=0)
ALIVE
--- 34 node A serves fn.matrix (rc=0)
{"groups": {"fn.matrix": "211 0 1 0 fn.matrix"}, "present": {}, "absent": {}, "ok": true}
--- 35 node A server is alive (rc=0)
ALIVE
--- 36 node A POST cycle (rc=0)
{"GROUP BEFORE": "211 0 1 0 fn.letters", "POST": "340 send article to be posted", "COMMIT": "240 article received OK", "GROUP AFTER": "211 1 1 1 fn.letters", "FRESH ARTICLE": "220 0 <native-matrix-c32cb94a7b82-a@example.invalid> article follows", "DUPLICATE POST": "340 send article to be posted", "DUPLICATE": "441 posting failed; the article was refused", "DATE AFTER DUPLICATE": "111 20260922013751", "counted": true, "ok": true}
--- 37 node A server is alive (rc=0)
ALIVE
--- 38 node A reader surface (rc=0)
{"greeting": "200 fn-nntp experimental server ready", "CAPABILITIES": "101 capability list follows", "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES HEADERS NEWSGROUPS OVERVIEW.FMT", "IMPLEMENTATION fn-nntp-lab", "IHAVE", "STREAMING"], "MODE READER": "200 posting allowed", "LIST ACTIVE": "215 list of active newsgroups follows", "LIST NEWSGROUPS": "215 list of newsgroups follows", "LIST OVERVIEW.FMT": "215 order of fields in overview database", "LIST ACTIVE.TIMES": "215 information follows", "LIST HEADERS": "215 field list follows", "GROUP": "211 1 1 1 fn.letters", "group_count": 1, "group_first": 1, "group_last": 1, "LISTGROUP": "211 1 1 1 fn.letters list follows", "ARTICLE": "220 1 <native-matrix-c32cb94a7b82-a@example.invalid> article follows", "HEAD": "221 1 <native-matrix-c32cb94a7b82-a@example.invalid> headers follow", "BODY": "222 1 <native-matrix-c32cb94a7b82-a@example.invalid> body follows", "STAT": "223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved", "ARTICLE MSGID": "220 0 <native-matrix-c32cb94a7b82-a@example.invalid> article follows", "ARTICLE ABSENT": "430 no article with that message-id", "OVER": "224 overview information follows", "OVER RANGE": "224 overview information follows", "HDR": "225 headers follow", "XOVER": "224 overview information follows", "XHDR": "221 header follows", "XPAT": "221 header follows", "NEXT": "421 no next article", "LAST": "422 no previous article", "DATE": "111 20260922013751", "HELP": "100 help text follows", "UNKNOWN": "500 command not recognized", "FRAMING": "111 20260922013751", "FRAMING SAME": true, "ok": true}
--- 39 node A server is alive (rc=0)
ALIVE
--- 40 install alpha.article (rc=0)
--- 41 node A outcome accepted (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article'
--- 42 install alpha.article (rc=0)
--- 43 node A outcome refused (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/alpha.article'
--- 44 install stream-a.article (rc=0)
--- 45 node A seed <stream-a@example.invalid> (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/a/stream-a.article'
--- 46 node B server is alive (rc=0)
ALIVE
--- 47 node B serves fn.matrix (rc=0)
{"groups": {"fn.matrix": "211 0 1 0 fn.matrix"}, "present": {}, "absent": {}, "ok": true}
--- 48 node B server is alive (rc=0)
ALIVE
--- 49 node B POST cycle (rc=0)
{"GROUP BEFORE": "211 1 1 1 fn.letters", "POST": "340 send article to be posted", "COMMIT": "240 article received OK", "GROUP AFTER": "211 2 1 2 fn.letters", "FRESH ARTICLE": "220 0 <native-matrix-4557f388c3ec-b@example.invalid> article follows", "DUPLICATE POST": "340 send article to be posted", "DUPLICATE": "441 posting failed; the article was refused", "DATE AFTER DUPLICATE": "111 20260922013753", "counted": true, "ok": true}
--- 50 node B server is alive (rc=0)
ALIVE
--- 51 node B reader surface (rc=0)
{"greeting": "200 fn-nntp experimental server ready", "CAPABILITIES": "101 capability list follows", "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES HEADERS NEWSGROUPS OVERVIEW.FMT", "IMPLEMENTATION fn-nntp-lab", "IHAVE", "STREAMING"], "MODE READER": "200 posting allowed", "LIST ACTIVE": "215 list of active newsgroups follows", "LIST NEWSGROUPS": "215 list of newsgroups follows", "LIST OVERVIEW.FMT": "215 order of fields in overview database", "LIST ACTIVE.TIMES": "215 information follows", "LIST HEADERS": "215 field list follows", "GROUP": "211 2 1 2 fn.letters", "group_count": 2, "group_first": 1, "group_last": 2, "LISTGROUP": "211 2 1 2 fn.letters list follows", "ARTICLE": "220 1 <native-matrix-c32cb94a7b82-a@example.invalid> article follows", "HEAD": "221 1 <native-matrix-c32cb94a7b82-a@example.invalid> headers follow", "BODY": "222 1 <native-matrix-c32cb94a7b82-a@example.invalid> body follows", "STAT": "223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved", "ARTICLE MSGID": "220 0 <native-matrix-4557f388c3ec-b@example.invalid> article follows", "ARTICLE ABSENT": "430 no article with that message-id", "OVER": "224 overview information follows", "OVER RANGE": "224 overview information follows", "HDR": "225 headers follow", "XOVER": "224 overview information follows", "XHDR": "221 header follows", "XPAT": "221 header follows", "NEXT": "223 2 <native-matrix-4557f388c3ec-b@example.invalid> retrieved", "LAST": "223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved", "DATE": "111 20260922013753", "HELP": "100 help text follows", "UNKNOWN": "500 command not recognized", "FRAMING": "111 20260922013753", "FRAMING SAME": true, "ok": true}
--- 52 node B server is alive (rc=0)
ALIVE
--- 53 install beta.article (rc=0)
--- 54 node B outcome accepted (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article'
--- 55 install beta.article (rc=0)
--- 56 node B outcome refused (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/beta.article'
--- 57 install stream-b.article (rc=0)
--- 58 node B seed <stream-b@example.invalid> (rc=4)
fault operator post [Errno 2] No such file or directory: '$HOME/fn-deploy/native915-783db50/b/stream-b.article'
--- 59 node A server is alive (rc=0)
ALIVE
--- 60 node B server is alive (rc=0)
ALIVE
--- 61 node B server is alive (rc=0)
ALIVE
--- 62 transit AB: offer <alpha@a.example.invalid> from A to B (rc=1)
{"msgid": "<alpha@a.example.invalid>", "source": "430 no article with that message-id", "ok": false, "transit": null, "reason": "the source node does not serve <alpha@a.example.invalid>"}
--- 63 node A server is alive (rc=0)
ALIVE
--- 64 transit BA: offer <beta@b.example.invalid> from B to A (rc=1)
{"msgid": "<beta@b.example.invalid>", "source": "430 no article with that message-id", "ok": false, "transit": null, "reason": "the source node does not serve <beta@b.example.invalid>"}
--- 65 node A server is alive (rc=0)
ALIVE
--- 66 node A capability pins (rc=0)
{"advertised": ["VERSION", "READER", "POST", "OVER", "HDR", "LIST", "IMPLEMENTATION", "IHAVE", "STREAMING"], "answered": {"READER": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "POST CLOSE": "441 posting failed; the article is not valid syntax", "IHAVE": "335 send it; end with <CR-LF>.<CR-LF>", "IHAVE CLOSE": "437 transfer rejected; not a valid article", "STREAMING": "203 streaming permitted", "OVER": "412 no newsgroup selected", "HDR": "412 no newsgroup selected", "LIST": "215 list of active newsgroups follows", "NEWNEWS": "500 command not recognized", "AUTHINFO": "381 password required", "STARTTLS": "580 can not initiate TLS negotiation", "MODE-READER": "200 posting allowed", "XOVER": "412 no newsgroup selected", "XHDR": "412 no newsgroup selected", "XPAT": "412 no newsgroup selected", "LISTGROUP": "211 2 1 2 fn.letters list follows", "CHECK": "238 <pin.check@matrix.example.invalid>", "TAKETHIS": "439 <pin.take@matrix.example.invalid>"}, "dispatched": ["READER", "POST", "IHAVE", "STREAMING", "OVER", "HDR", "LIST", "AUTHINFO", "STARTTLS", "MODE-READER", "XOVER", "XHDR", "XPAT", "LISTGROUP", "CHECK", "TAKETHIS"], "login": {}, "advertised_not_dispatched": [], "dispatched_not_advertised": [], "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES HEADERS NEWSGROUPS OVERVIEW.FMT", "IMPLEMENTATION fn-nntp-lab", "IHAVE", "STREAMING"], "ok": true}
--- 67 node B server is alive (rc=0)
ALIVE
--- 68 node B capability pins (rc=0)
{"advertised": ["VERSION", "READER", "POST", "OVER", "HDR", "LIST", "IMPLEMENTATION", "IHAVE", "STREAMING"], "answered": {"READER": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "POST CLOSE": "441 posting failed; the article is not valid syntax", "IHAVE": "335 send it; end with <CR-LF>.<CR-LF>", "IHAVE CLOSE": "437 transfer rejected; not a valid article", "STREAMING": "203 streaming permitted", "OVER": "412 no newsgroup selected", "HDR": "412 no newsgroup selected", "LIST": "215 list of active newsgroups follows", "NEWNEWS": "500 command not recognized", "AUTHINFO": "381 password required", "STARTTLS": "580 can not initiate TLS negotiation", "MODE-READER": "200 posting allowed", "XOVER": "412 no newsgroup selected", "XHDR": "412 no newsgroup selected", "XPAT": "412 no newsgroup selected", "LISTGROUP": "211 2 1 2 fn.letters list follows", "CHECK": "238 <pin.check@matrix.example.invalid>", "TAKETHIS": "439 <pin.take@matrix.example.invalid>"}, "dispatched": ["READER", "POST", "IHAVE", "STREAMING", "OVER", "HDR", "LIST", "AUTHINFO", "STARTTLS", "MODE-READER", "XOVER", "XHDR", "XPAT", "LISTGROUP", "CHECK", "TAKETHIS"], "login": {}, "advertised_not_dispatched": [], "dispatched_not_advertised": [], "capability_lines": ["VERSION 2", "READER", "POST", "OVER MSGID", "HDR", "LIST ACTIVE ACTIVE.TIMES HEADERS NEWSGROUPS OVERVIEW.FMT", "IMPLEMENTATION fn-nntp-lab", "IHAVE", "STREAMING"], "ok": true}
--- 69 node A server is alive (rc=0)
ALIVE
--- 70 node B server is alive (rc=0)
ALIVE
--- 71 a second reader across node A's POST (rc=0)
{"WATCHER BEFORE": "211 2 1 2 fn.letters", "POST": "340 send article to be posted", "WATCHER MID": "211 2 1 2 fn.letters", "WATCHER ARTICLE MID": "223 1 <native-matrix-c32cb94a7b82-a@example.invalid> retrieved", "COMMIT": "240 article received OK", "WATCHER AFTER": "211 2 1 2 fn.letters", "ok": true}
--- 72 node A server is alive (rc=0)
ALIVE
--- 73 node B server is alive (rc=0)
ALIVE
--- 74 offline group create while the owner holds the store (rc=1)
refused operator group store is already locked
--- 75 native public peering/restart witness (rc=0)
...
----------------------------------------------------------------------
Ran 2 tests in 4.647s

OK
--- 76 stop server node A (main) (rc=0)
stopped
--- 77 node A store after the stop (rc=0)
transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment
accepted operator STATUS
--- 78 node A recover after the stop (rc=0)
recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none
accepted operator RECOVER
--- 79 node A log tail (rc=0)
CONTROL /home/ember/fn-native915-matrix/a/control.sock
LISTENING 11190
accepted operator run
--- 80 stop server node B (main) (rc=0)
stopped
--- 81 node B store after the stop (rc=0)
transactions=3 articles=3 staging-orphans=0 unsigned-legacy-experiment
accepted operator STATUS
--- 82 node B recover after the stop (rc=0)
recovered transactions=3 articles=3 staging-orphans=0 anchor=none checkpoint=none
accepted operator RECOVER
--- 83 node B log tail (rc=0)
CONTROL /home/ember/fn-native915-matrix/b/control.sock
LISTENING 11191
accepted operator run
--- 84 peer remove a peer that is not there (rc=1)
refused operator peer administrative configuration refused: NO-SUCH-PEER
--- 85 peer remove the configured peer (rc=0)
configured generation=6 record=00000006.cfg verification=VERIFIED
accepted operator peer
--- 86 stop server node A (main) (rc=0)
stopped
--- 87 stop the tap in front of node A (rc=0)
stopped
--- 88 stop server node B (main) (rc=0)
stopped
--- 89 stop the tap in front of node B (rc=0)
stopped
--- 90 stray fn processes (rc=0)
CLEAN
--- 91 remove the deploy tree (rc=0)
--- 92 release deploy lock (rc=0)
```
