#!/bin/sh
# control-c3b: build the developer image from the lane tree and run the native control filing cases (signed cancel filed; the served withdrawal, test_signed_withdrawal).
set -u
T=$1
...
grep -E "NATIVE-CONTROL-WITNESS|NATIVE-WITHDRAWAL-WITNESS|^(OK|FAILED)|Error|ok$|FAIL" build/lane/native-control-filing.log
