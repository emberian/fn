...
set -u
T=$1
B=/tank/fn/scratch/operator-walk
W=$B/run
P=$B/prefix
export FN_OPENSSL_PREFIX=/tank/fn/toolchains/openssl-3.5.8
export LD_LIBRARY_PATH=$FN_OPENSSL_PREFIX/lib
for u in a b a2 copy nostream; do systemctl --user stop fn-ow-$u 2>/dev/null; systemctl --user reset-failed fn-ow-$u 2>/dev/null; done
...
systemd-run --user --unit fn-ow-nostream -p MemoryMax=1G python3 $T/planning/evidence/operator-walk-2026-09-26/nostream_peer.py 31603 > /dev/null 2>&1
sleep 1; start a
art 2; step post-a-2 0 $FN operator $A post --message-id '<walk-2@example.invalid>' --payload $W/p/2 --group local.test
sleep 40
journalctl --user -u fn-ow-nostream --no-pager -o cat > $O/nostream.log 2>&1
...
systemctl --user stop fn-ow-nostream
