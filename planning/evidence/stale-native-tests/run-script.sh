#!/bin/sh
# stale-native-tests runner (from the native-subsets-6c0626c5 runner): one module (or test id) at a time, each to its own log.
# Usage: sh run.sh target [target...]   (runs in the scratch copy of the gate)
# NS_TAG=x adds .x to the log name. The image's SBCL is first on PATH and every
# opt-in end-to-end flag is set from the start.
set -u
G=/tank/fn/gates/qual-6c0626c5-20260924
I=$G/build/images/6c0626c5
S=/tank/fn/scratch/stale-native-tests
T=$S/tree
L=$S/logs
mkdir -p "$L"
cd "$T"
...
export FN_NATIVE_BP_HOST=$I/${NS_BP_IMAGE:-fn-host-dtn}   # NS_BP_IMAGE=fn-host-dtn-developer for the selector cases
export FN_NATIVE_CONTACT_SENDER=$I/fn-host-dtn
export FN_NATIVE_CONTACT_RECEIVER=$I/fn-host-dtn
export FN_NATIVE_IMAGE_SOURCE_SHA=6c0626c5ad5691966fdbbae475dcd886ec425f28
...
for m in "$@"; do
  log=$L/$(basename $m .py)${NS_TAG:+.$NS_TAG}.log
  {
  ...
  s=$(date +%s.%N)
  PYTHONPATH=$T timeout 300 python3 "$m" >> "$log" 2>&1
  rc=$?
  e=$(date +%s.%N)
