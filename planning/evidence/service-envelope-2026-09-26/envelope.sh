...
# Both copies drop the derived checkpoint: their first open is the full
# replay; `checkpoint,restart` then gives the open from a fresh checkpoint.
unit clone10k-$TAG "$T clone --drop-checkpoint --dir /dev/shm/senv/src10k-$TAG --to-dir /dev/shm/senv/t10k-$TAG && $T clone --drop-checkpoint --dir /dev/shm/senv/src10k-$TAG --to-dir $S/z10k-$TAG"
...
# The open as the store stands (its own auto checkpoint and suffix), then a
# fresh checkpoint and the open from it, then the rates; last, the full
# replay on a copy without the checkpoint (the case most likely to exceed a unit).
