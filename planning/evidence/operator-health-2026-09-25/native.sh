...
health() { # LABEL CONFIG
  ...
  local code=$?
  say "$label exit=$code first=$(head -1 $O/$label.out) held=[$(grep ' held' $O/$label.out | cut -d' ' -f1 | tr '\n' ' ')] unobserved=[$(grep ' unobserved' $O/$label.out | cut -d' ' -f1 | tr '\n' ' ')]"
}
...
node dev 12101 development
health unqualified-offline $W/dev/fn.toml

# 2. space-pressure (23), offline: scale profile, headroom_min_percent 100.
node scale 12102 scale
...
fnop $W/scale/fn.toml peer add hub hub.example 127.0.0.1 12199 - 'local.*' false > $O/peer-add.out 2>&1; say "peer add exit=$? $(tail -1 $O/peer-add.out)"
start scale
...
kill -STOP $PID; health fenced-unanswering $W/scale/fn.toml; kill -CONT $PID
stop scale

# 5. stranded-transfer (25), live: a peer that answers 436 to every offer.
node strand 12103 scale
python3 $T/planning/evidence/operator-health-2026-09-25/defer_peer.py 12198 > $O/defer-peer.log 2>&1 &
DP=$!
fnop $W/strand/fn.toml peer add sink sink.example 127.0.0.1 12198 - 'local.*' false > $O/peer-add-sink.out 2>&1; say "peer add sink exit=$?"
start strand
...
fnop $W/fwd/fn.toml bp-boundary add fn-b fn-b.example dtn://fn-b/ 12197 > $O/bpb.out 2>&1; say "bp-boundary add exit=$? $(tail -1 $O/bpb.out)"
