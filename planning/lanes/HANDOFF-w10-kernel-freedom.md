...
| `run-20260920T214551Z-90a4` | the FINAL tree (`--affected-by books/store-files.lisp --affected-by books/byte-store-scan.lisp --closure`), covering the `:rule-classes nil` and byte-store-scan-comment commits | 129 | **125 passed, 4 failed**, 269.9 s, evidence `build/acl2/certify-20260920T214556Z-3646511`. The four are the same pre-existing ones: `books/owner-invariants`, `books/owner-config`, `tests/acl2/owner-tests`, `tests/acl2/checkpoint-codec-tests`. |
