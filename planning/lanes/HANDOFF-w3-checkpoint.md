...
proofs were written against, and each fix uncovers the next form. Eleven closed, one open (rows 8 to 11 by lane `w9/storage` on 2026-09-20,
...
| 12 | **open**: `fn-cpc-accepted-input-is-canonical` | Subgoal 105.76': the octet-ness of `fn-cpc-read-strings`' remainder, over a count read out of the header, is not available | the same shape as #9's second rule: a forward-chaining `fn-cpc-read-strings-domain` on the trigger term `(fn-record-parse-rest (fn-cpc-read-strings n octets))`, so the `e/d` need not open the reader | `build/probe/d2.out:6027` on hbox (ld probe, not a certification) |
