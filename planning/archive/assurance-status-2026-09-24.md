...
branches is not counted as integrated source.
...
`e160442f`. The later `8c61c098` and `f0d67034` combined runs failed. The
frozen `67d026ad` run then certified 263 new books but failed on four direct
theorems and seven missing-certificate dependents; it produced no image.
Consumer/topic projection has since been scoped-certified, and the
acceptance-stamp and Store-prepare hint repairs have scoped certificates.
The BP observed-reopen repair is conditional on topic replay; the maintained
topic/crash bridge remains open. Source at `a7184f37` is newer than the
qualified image. The protected live service has not been upgraded by this
cycle.
...
- An acknowledged article has a durable history entry and survives modeled
  ...
  callback-to-Store frontier projection and one applied root-directory EIO cut
  have scoped certification: on that modeled error, the candidate can be
  durable while the owner fences and reports uncertainty. Establishing the
  byte/kernel relation at every served call entry and across all native
...
- Consumer progress is durable and scoped to a Store history/incarnation,
  principal and view. Polling does not acknowledge processing. The maintained
  consumer relation now has a certified mixed article/topic/reopen trace in
  which topic records advance the frontier but leave application ACK at zero.
  Mini must commit its own durable transaction before advancing fn's cursor.
...
| Storage and recovery | Conditional crash scanning/reopen/acknowledged-record survival, native process-death and isolated filesystem-EIO evidence. The shared-owner successful frontier callback has an exact ACL2 owner-to-Store projection and a four-event relation witness. A selected modeled root-directory `:eio :apply` cut is certified through the actual owner callback; its matching `:drop` witness shows why EIO cannot imply persistence. | Establish and preserve the byte/kernel relation at every served call entry, including other error and torn-barrier cuts; then compose compaction/recovery and qualify physical fsync behavior. The modeled applied cut is not a native fsync-EIO qualification. |
| BP/DTN | Earlier native whole-bundle/contact/application-receipt paths and deletion-report recovery tested. Fragment custody plus persisted expiry-safe reassembly and a host-called route-wait/younger-local progress slice are integrated with scoped ACL2 evidence. | A source-matched interrupted-fragment/restart/dispatch/receipt image run is pending. N03's full four-class selection, N04 forwarding/session policy and N05 durable debt remain open. BP receipt recovery across observed topic reopen currently has an explicit topic-replay premise; deriving it from reachable maintained Store state is still owed. |
| Durable consumption | Native poll, unchanged ACK on repeat, durable advancing ACK/lost-reply/reopen and cursor inspection/status tests on named earlier images. The maintained Store relation and reachable mixed consumer/topic/reopen trace are scoped-certified, including unchanged application ACK. | Indexed owner poll and joined status need a new source-matched image/cost gate. The mixed trace is proof-side composition, not served-runtime validation or a general consumer transaction/ACK join. |
...
| Topic histories | The experimental local administrator/root/report Store path has scoped historical source binding, roster/parent/quota and reopen proofs. The actual owner-called report selector now gives exact admitted-source retries a distinct success without another event or charge. | The combined native install/anchor/report/reopen/retry fixture has not run on a source-matched image. Integrated version-1 anchors bind the installed administrator ID but omit its separate installation generation; version-2 migration is agent-branch work, not a main-branch assurance result. Portable succession/governance is outside this fixed local profile. |
| Preservation and efficiency | Selected pack/reclaim/cold-clone tests, new incarnation refusing old cursors, indexed Message-ID/LISTGROUP/OVER paths; bounded consumer event index source. A read-only native retention diagnostic and active-reader exclusion fixture are integrated source. | The active-reader/reclaim/reopen fixture has not run on a matching image. Full reclamation/crash correspondence, pruning policy, large-workload measurement and remaining scans stay open. Mini replay cost also remains material. |
...
| Assurance engineering | Source/closure-aware cache, generated ledger, scoped native evidence and bounded proof iteration. The called retention prepare guard is verified under `fn-sn-statep`. Store and pinned-peer proof-only repairs reduced named book costs on measured runs without changing theorem statements. | The `67d026ad` combined gate failed with four direct theorem reds and seven dependent missing certificates. Acceptance-stamp and Store-prepare hint repairs are scoped-certified, but a new combined gate and matching image are still required. Close maintained-guard/witness and reverse-closure gaps, and measure whole workflows. Proof speedups are iteration results, not new runtime correctness claims. |
...
matter, but only the first changes behavior. The `f0d67034` gate narrowed
its direct failures to consumer Store projection and configuration live state;
both were repaired with scoped certificates. The later `67d026ad` gate exposed
four different direct proof failures. Its acceptance-stamp and Store-prepare
hint repairs have scoped certificates, while the BP observed-reopen statement
now explicitly requires topic replay. The needed maintained topic/crash bridge
must be proved for reachable images, not inferred from a test of malformed
records. A combined gate must still pass before a matching image is built.
