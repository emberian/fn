             ...
             (progn
               ...
               (let ((adu (fnn-receipt-adu journal (fourth view))))
                 ...
                 (let* ((peer (fnn-bp-eid (fifth view)))
                      ;; Queue time: ACL2's route to the receipt's routed
                      ;; hop (fn-bprt-job-route over the owner's table).
                      (route (fnn-bps-queue-route bp peer node-id transfer-mru
                      ...
                      (payload (and route
                                    (fnn-bpnode-signed-receipt adu (fifth view))))
                      (sequence (and route
                                     (fnn-bp-reserve-sequence (fnn-bps-tally bp))))
                      (observation (fnn-bp-observation wall wall-error)))
                 (unless route
                   ;; Not queued: the receipt stays owed in FNRJ and a later
                   ;; pass queues it once the table routes its destination.
                   (return-from fnn-bpnode-queue-outbox :no-route))
                 (fnn-bps-drive-effects
                  bp (fnn-bps-step
...
(defun fnn-bpnode-route-by-owner (bp)
  "Routing of this node's base jobs is in force: ACL2's route table of the
owner's current configuration (fn-owner-bp-route-table), read each pass so
an operator's `bp-route' change applies to the next queue and contact."
  (setf (fnn-bps-routing bp)
        (list :table (fnn-owner-core 'fn-owner-bp-route-table)))
  bp)
...
(defun fnn-bpnode-queue-report
  ...
  (let* ((work (fifth view))
    ...
    (let* ((route (fnn-bps-queue-route bp (fnn-bp-eid peer-id) node-id
                                       transfer-mru contact-host contact-port))
           (sequence (and route (fnn-bp-reserve-sequence (fnn-bps-tally bp))))
           (observation (fnn-bp-observation wall wall-error)))
      (unless route
        ;; Not queued: the report intent stays and a later pass queues it.
        (return-from fnn-bpnode-queue-report :no-route))
      (fnn-bps-drive-effects
       bp (fnn-bps-foundation-step
...
scope (spec bp-node-machine 4.3.2): a refused, failed or uncertain transfer
is connection-local.  ACL2's durable :requeued record keeps the job owed
under its own identity (fn-bpnp-uncertain-receipt-transfer-keeps-the-job-
owed) and the next contact offers it again (fn-bpnp-receipt-reoffer-after-
...
uncertain, as it does everywhere else."
  ...
  (let* ((peer (fnn-bp-eid peer-id))
    ...
    (unless (eq (first first-answer) :close)
      ...
      (unwind-protect (fnn-bpc-drive-contact bp (list :contact peer t))
