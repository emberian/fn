...
| [Wildmat](../books/wildmat.lisp) | Bounded strict UTF-8 grammar and dynamic-programming matching, reused for filtered LIST variants | UTF-8 progress/scalars, successful parsing and DP/reference correspondence proved; full parser/matcher work and guards remain open |
...
| [File-publication kernel](../books/store-files.lisp) and [invariants](../books/store-files-invariants.lisp) | One-use allocator reservations, immutable publication, actual replay and recovery gates; transition/crash preservation, stable-prefix and one-crash success retention proved | Finite file traces preserve state/history/successes; live node composition is separately proved; actual host adoption/refinement remains open |
| [Live file/node composition](../books/store-node.lisp) | Fixed configuration, exact pending-record binding, actual node completion before acknowledgement and full-node replay extension | Mixed-trace and observed-image work are tracked separately from this checkpoint; actual adapter adoption is in progress |
...
- Wire event-yield accounting and state preservation are certified. CBOR/record
  guards are verified; other runtime guards, complete session refinement and
  remaining parser work proofs are active closure tasks.
