...
| [Wildmat](../books/wildmat.lisp) | Bounded strict UTF-8 grammar and dynamic-programming matching, reused for filtered LIST variants | UTF-8 progress/scalars, successful parsing and DP/reference correspondence proved; whole matcher work and all 54 guards have targeted proofs; parser work remains open |
...
- Wire event-yield accounting and state preservation are certified. Nine complete runtime graphs have targeted guard evidence (344 functions).
  Implemented NNTP session/cursor preservation and effect typing are certified;
  remaining runtime graphs, full RFC session refinement and parser work remain active.
