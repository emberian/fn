...
| [NNTP reader](../books/nntp.lisp) | Experimental reader commands including LISTGROUP ranges/cursors, filtered LIST ACTIVE/NEWSGROUPS and NEWNEWS over committed state, with independent response transcripts | Incomplete READER bundle; no signed injection. NEWNEWS reads the article's own Injection-Date or Date -- the store keeps no arrival stamp -- and refuses past its 256-article parse budget ([the bound and its scope](../specs/nntp.md#polling-newnews)). POST is [`books/nntp-post.lisp`](../books/nntp-post.lisp) and RFC 4643 authentication is [`books/nntp-auth.lisp`](../books/nntp-auth.lisp), both above this book; the credential and the posting allowance reach a RUNNING server as of 2026-09-21 ([auth-live](../planning/evidence/auth-live-2026-09-21.md)) |
| [AUTHINFO and STARTTLS](../books/nntp-auth.lisp) | RFC 4643 USER/PASS against a salted verifier ACL2 derives, RFC 4642 STARTTLS around a host handshake, the 480 gate and the per-principal posting allowance, all pinned once per connection | The secret crosses the wire in the clear unless `[auth] protected_only` and a certificate are configured; no SASL mechanism; a transit peer cannot authenticate, so `[auth] required` also gates `IHAVE` (board, w11/auth-live) |
